...
export default function AppMenu({ homeTab, setHomeTab, toolKey, setToolKey, titel, rechts }) {
  ...
  /* Nur die Ebene zeigen – den Tool-Namen nennt das Zurück-Band in ReiseTools bereits. */
  const label = homeTab === "tools" ? "Tools" : (HOME.find((h) => h.k === homeTab) || {}).label || "";
  ...
  return (
    <div className="mb-3">
      ...
      {open && (
        <div className="mt-2 rounded-2xl border border-stone-200 bg-white p-2 shadow-lg dark:border-stone-700 dark:bg-stone-900">
          {/* Home-Bereiche */}
          {HOME.map((h) => {
