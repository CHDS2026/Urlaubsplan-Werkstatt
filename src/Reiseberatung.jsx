...
import { MONATE_KURZ as MONATE } from "./lib/datum.js";

const WV = "https://de.wikivoyage.org";
const WP = "https://de.wikipedia.org";
const enc = encodeURIComponent;
const CELLE = { lat: 52.6226, lon: 10.0806 };
const INPUT = "w-full rounded-lg border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none";
...
/* ---- component ---- */
export default function Reiseberatung({ onAdd, defaultQuery = "", region = "", embedded = false }) {
  ...
  return (
    <section className={embedded ? "space-y-0" : "rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-white dark:bg-stone-900 p-4 shadow-sm"}>
      {!embedded && (
        <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between">
          ...
          {open ? <ChevronUp className="h-4 w-4 text-stone-400 dark:text-stone-500" /> : <ChevronDown className="h-4 w-4 text-stone-400 dark:text-stone-500" />}
        </button>
      )}
      ...
      {(open || embedded) && (
        <div className="mt-3 space-y-3">
          {!embedded && (<>
          <p className="text-xs text-stone-500 dark:text-stone-400">Steckbrief zu Region/Stadt aus Wikivoyage &amp; Wikipedia – mit Fakten, Fahrzeit ab Celle, bester Reisezeit, Bergbahnen und Sehenswürdigkeiten zum Übernehmen. Frei, ohne Schlüssel.</p>
          ...
          {loading && !data && <div className="flex items-center gap-2 rounded-xl bg-stone-50 dark:bg-stone-800 px-3 py-3 text-sm text-stone-500 dark:text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> Reiseführer wird geladen …</div>}
            ...
            <div className="space-y-3">
              ...
              <div className="overflow-hidden rounded-xl border border-stone-200 dark:border-stone-700">
                ...
                <div className="p-3">
                  ...
                  <h4 className="mt-1 text-lg font-bold text-stone-900 dark:text-stone-100">{data.title}</h4>
                  {data.desc && <div className="text-xs italic text-stone-500 dark:text-stone-400">{data.desc}</div>}

                  {/* Fakten + Fahrzeit */}
                  <div className="mt-2 flex flex-wrap gap-2">
                    {fahr.loading && <span className="inline-flex items-center gap-1 rounded-lg bg-stone-100 dark:bg-stone-800 px-2 py-1 text-xs text-stone-400 dark:text-stone-500"><Loader2 className="h-3 w-3 animate-spin" /> Fahrzeit …</span>}
                    ...
                    {facts.data && facts.data.einwohner != null && <span className="inline-flex items-center gap-1 rounded-lg bg-stone-100 dark:bg-stone-800 px-2 py-1 text-xs text-stone-600 dark:text-stone-300"><Users className="h-3.5 w-3.5" /> {nf(Math.round(facts.data.einwohner))} Einw.</span>}
                    {facts.data && facts.data.hoehe != null && <span className="inline-flex items-center gap-1 rounded-lg bg-stone-100 dark:bg-stone-800 px-2 py-1 text-xs text-stone-600 dark:text-stone-300"><TrendingUp className="h-3.5 w-3.5" /> {nf(Math.round(facts.data.hoehe))} m</span>}
                    {facts.data && facts.data.website && <a href={facts.data.website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-stone-100 dark:bg-stone-800 px-2 py-1 text-xs font-medium text-emerald-700 hover:text-emerald-800"><Globe className="h-3.5 w-3.5" /> Website</a>}
                  </div>

                  {data.intro && <p className="mt-2 text-sm leading-relaxed text-stone-700 dark:text-stone-200">{data.intro}</p>}
              ...
              {(klima.loading || klima.data) && (
                <div className="rounded-xl border border-stone-200 dark:border-stone-700 p-3">
                  <div className="mb-2 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700 dark:text-stone-200"><Sun className="h-4 w-4 text-amber-500" /> Beste Reisezeit</div>
                  {klima.loading && <div className="flex items-center gap-2 text-sm text-stone-400 dark:text-stone-500"><Loader2 className="h-4 w-4 animate-spin" /> Klima wird ausgewertet …</div>}
                  {klima.data && (
                    <>
                      <div className="grid grid-cols-6 gap-1">
                        {klima.data.map((m, i) => (
                          <div key={i} className="rounded-md px-1 py-1 text-center" style={{ backgroundColor: tempColor(m.t) }}>
                            <div className="font-medium text-stone-700 dark:text-stone-200" style={{ fontSize: "10px" }}>{MONATE[i]}</div>
                            <div className="text-xs font-bold text-stone-800 dark:text-stone-100">{m.t == null ? "–" : Math.round(m.t) + "°"}</div>
                          </div>
                        ))}
                      </div>
                      <div className="mt-2 text-xs text-stone-500 dark:text-stone-400">Angenehm mild: <b className="text-stone-700 dark:text-stone-200">{besteZeit(klima.data)}</b> · Ø-Temperatur, Klimamittel 2023–2025.</div>
                    </>
                  )}
                </div>
              )}
              ...
              <div>
                <div className="mb-2 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700 dark:text-stone-200"><Landmark className="h-4 w-4 text-emerald-700" /> Top-Sehenswürdigkeiten</div>
                {data.sights.length === 0 && <p className="text-sm text-stone-400 dark:text-stone-500">Keine strukturierten Sehenswürdigkeiten gefunden – schau in den Artikel oben.</p>}
                <ul className="space-y-2">
                  {data.sights.map((s, i) => {
                    const isAdded = !!added[s.name];
                    return (
                      <li key={i} className="flex items-start gap-3 rounded-xl border border-stone-100 dark:border-stone-800 p-3">
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-semibold text-stone-800 dark:text-stone-100">{s.name}</div>
                          {s.description && <div className="mt-0.5 text-xs text-stone-500 dark:text-stone-400">{s.description}</div>}
              ...
              {(berg.loading || (berg.data && berg.data.length > 0)) && (
                <div>
                  <div className="mb-2 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700 dark:text-stone-200"><Mountain className="h-4 w-4 text-emerald-700" /> Bergbahnen in der Nähe</div>
                  {berg.loading && <div className="flex items-center gap-2 text-sm text-stone-400 dark:text-stone-500"><Loader2 className="h-4 w-4 animate-spin" /> Seilbahnen werden gesucht …</div>}
                  <ul className="space-y-2">
                    {berg.data.map((s, i) => {
                      const isAdded = !!added[s.name];
                      return (
                        <li key={i} className="flex items-center gap-3 rounded-xl border border-stone-100 dark:border-stone-800 p-3">
                          <span className="flex-1 text-sm font-medium text-stone-800 dark:text-stone-100">{s.name}</span>
              ...
              <div className="flex items-start gap-2 text-xs text-stone-400 dark:text-stone-500"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Quellen: {data.hasWV ? "Wikivoyage & Wikipedia" : "Wikipedia"} (CC BY-SA), Wikidata, Open-Meteo, OSM · Fahrzeit über OSRM-Demo (Richtwert).</span></div>
            </div>
          )}

          {/* Schulferien Niedersachsen – Reise-Timing */}
          <div className="rounded-xl border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800 p-3">
            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700 dark:text-stone-200"><CalendarRange className="h-4 w-4 text-emerald-700" /> Schulferien Niedersachsen</div>
            <div className="mt-0.5 text-xs text-stone-400 dark:text-stone-500">Fürs Reise-Timing (Andrang &amp; Preise) – Start bei dir zuhause.</div>
            {ferien.loading && <div className="mt-2 flex items-center gap-2 text-sm text-stone-400 dark:text-stone-500"><Loader2 className="h-4 w-4 animate-spin" /> lädt …</div>}
            <div className="mt-2 flex flex-wrap gap-1.5">
              {ferien.data.map((f, i) => (
                <span key={i} className="inline-flex items-center gap-1 rounded-lg bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 px-2 py-1 text-xs text-stone-600 dark:text-stone-300">
                  <b className="text-stone-800 dark:text-stone-100">{f.name}</b> {dm(f.start)}–{dm(f.end)}{String(f.end).slice(0, 4)}
                </span>
              ))}
              {!ferien.loading && ferien.data.length === 0 && <span className="text-xs text-stone-400 dark:text-stone-500">keine Daten</span>}
            </div>
          </div>
        </div>
      )}
