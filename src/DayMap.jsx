...
export default function DayMap({ punkte, routeGeojson, compact }) {
  ...
  // Routenlinie zeichnen/aktualisieren (wird erst gesetzt, wenn der Style geladen ist)
  useEffect(() => {
    const k = karte.current;
    if (!k) return;
    const apply = () => {
      if (!karte.current) return;
      const data = routeGeojson || LEER;
      const src = k.getSource("tagesroute");
      if (src) { src.setData(data); return; }
      k.addSource("tagesroute", { type: "geojson", data });
      k.addLayer({
        id: "tagesroute-linie",
        type: "line",
        source: "tagesroute",
        layout: { "line-join": "round", "line-cap": "round" },
        paint: { "line-color": "#047857", "line-width": 4, "line-opacity": 0.75 },
      });
    };
    if (k.isStyleLoaded()) apply();
    else k.once("load", apply);
  }, [routeGeojson]);

  return <div ref={box} className={`${compact ? "h-44" : "h-64"} w-full overflow-hidden rounded-xl border border-stone-200`} />;
}
