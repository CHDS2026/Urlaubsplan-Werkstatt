...
import {
  ...
  ChevronDown, ChevronUp, Plane, Link2, ExternalLink, Star, RotateCcw, Thermometer
} from "lucide-react";
...
const Reisezeit = React.lazy(() => import("./Reisezeit.jsx"));
...
export default function Reiseziele({ wishlist, setWishlist, visited, setVisited, regions, setRegions, visitedRegions, setVisitedRegions, wishRegions, setWishRegions, trips, onCreateTripFromWish, onOpenTrip }) {
  const [tab, setTab] = useState("wunsch");
  const tabs = [
    { key: "wunsch", label: "Wunschliste", icon: Heart },
    { key: "zeit", label: "Reisezeit", icon: Thermometer },
  ...
  return (
    <div>
      ...
      {tab === "zeit" && (
        <Suspense fallback={<div className="rounded-2xl border border-stone-200 bg-white p-10 text-center text-sm text-stone-500">lädt …</div>}>
          <Reisezeit wishlist={wishlist} />
        </Suspense>
      )}
      {tab === "welt" && <WeltkarteView visited={visited} setVisited={setVisited} />}
      {tab === "europa" && <EuropaView regions={regions} setRegions={setRegions} visitedRegions={visitedRegions} setVisitedRegions={setVisitedRegions} wishRegions={wishRegions} setWishRegions={setWishRegions} />}
    </div>
  );
}
...
function EuropaView({ regions, setRegions, visitedRegions, setVisitedRegions, wishRegions, setWishRegions }) {
  ...
  // Doppelklick: besucht (grün). Einfacher Klick: Wunsch (pink).
  const toggleBesucht = (id, name) => {
    const next = { ...(visitedRegions || {}) };
    if (next[id]) delete next[id]; else next[id] = name || true;
    setVisitedRegions(next);
    // besucht schlägt Wunsch: dann Wunsch entfernen
    if (next[id] && wishRegions && wishRegions[id]) { const w = { ...wishRegions }; delete w[id]; setWishRegions(w); }
  };
  const toggleWunsch = (id, name) => {
    if (visitedRegions && visitedRegions[id]) return; // besuchte Region bleibt grün
    const next = { ...(wishRegions || {}) };
    if (next[id]) delete next[id]; else next[id] = name || true;
    setWishRegions(next);
  };
  const mapCount = Object.keys(visitedRegions || {}).filter((k) => visitedRegions[k]).length;
  const wishCount = Object.keys(wishRegions || {}).filter((k) => wishRegions[k]).length;
  const resetMap = () => { if (window.confirm("Alle Markierungen auf der Karte (besucht und Wunsch) zurücksetzen?")) { setVisitedRegions({}); setWishRegions({}); } };
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="flex flex-wrap gap-2">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-sm font-semibold text-emerald-700"><MapIcon className="h-4 w-4" /> {mapCount} besucht</span>
              {wishCount > 0 && <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-100 px-3 py-1 text-sm font-semibold text-rose-700"><Heart className="h-4 w-4" /> {wishCount} Wunsch</span>}
            </span>
            {(mapCount > 0 || wishCount > 0) && <button onClick={resetMap} className="inline-flex items-center gap-1 text-xs font-medium text-stone-400 hover:text-rose-500"><RotateCcw className="h-3.5 w-3.5" /> zurücksetzen</button>}
          </div>
          <MapBoundary fallback={<div className="rounded-xl border border-dashed border-stone-300 bg-white p-6 text-center text-sm text-stone-500">Die Regionen-Karte konnte nicht geladen werden. Beim ersten Mal ist dafür eine Internetverbindung nötig – danach geht es offline. Über „Liste" kannst du Regionen trotzdem abhaken.</div>}>
            <Suspense fallback={<div className="rounded-xl border border-stone-200 bg-sky-100 p-10 text-center text-sm text-stone-500">Regionen-Karte lädt … (kann beim ersten Mal etwas dauern)</div>}>
              <EuropeMap visited={visitedRegions || {}} wish={wishRegions || {}} onSingle={toggleWunsch} onDouble={toggleBesucht} />
            </Suspense>
          </MapBoundary>
          <div className="flex flex-wrap items-center gap-3 text-xs text-stone-500">
            <span className="inline-flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm" style={{ backgroundColor: "#059669" }} /> besucht (Doppeltipp)</span>
            <span className="inline-flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm" style={{ backgroundColor: "#ec4899" }} /> Wunsch (einfacher Tipp)</span>
          </div>
          <p className="text-xs text-stone-400">Zwei Finger zum Zoomen. Nochmal tippen entfernt die Markierung wieder. Hinweis: Karte und „Liste" werden getrennt gezählt.</p>
        </div>
      ) : (
        <>
          <div className="flex items-center justify-between">
