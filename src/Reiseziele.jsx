...
import { EUROPE_REGIONS, regionKey, europeCountries } from "./geo.js";
...
export default function Reiseziele({ wishlist, setWishlist, visited, setVisited, regions, setRegions, trips, onCreateTripFromWish, onOpenTrip }) {
  ...
  return (
    <div>
      ...
      {tab === "wunsch" && <Wishlist wishlist={wishlist} setWishlist={setWishlist} trips={trips} onCreateTripFromWish={onCreateTripFromWish} onOpenTrip={onOpenTrip} />}
      {tab === "welt" && <WeltkarteView visited={visited} setVisited={setVisited} />}
      {tab === "europa" && <EuropaView regions={regions} setRegions={setRegions} />}
    </div>
  );
}
...
function Wishlist({ wishlist, setWishlist, trips, onCreateTripFromWish, onOpenTrip }) {
...
function EuropaView({ regions, setRegions }) {
  const [q, setQ] = useState("");
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-sm font-semibold text-emerald-700"><MapIcon className="h-4 w-4" /> {totalCount} Regionen besucht</span>
      </div>
