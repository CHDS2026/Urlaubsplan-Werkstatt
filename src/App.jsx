...
import {
  ...
  Paperclip, FileText, Image as ImageIcon, Globe2, Search, Map as MapIcon, Route, Thermometer, Droplets, Loader2, CalendarPlus, PartyPopper, Crosshair, ChevronRight, ShieldAlert, LayoutGrid, MountainSnow, Bike, Sparkles
} from "lucide-react";
...
import { verteileAufTage } from "./lib/tagesplan.js";
import { isValidISO, addDays } from "./lib/datum.js";
import { abstandKmPunkte } from "./lib/abstand.js";
...
async function ladeAttraktionen(lat, lon) {
  const R = 15000;
  // Zwei Ebenen, damit bekannte Ziele nie durch das Ergebnis-Limit verdrängt werden:
  //  1) alles mit Wikidata (verlässlich bekannte Ziele – kommen garantiert)
  //  2) zusätzlich namentliche Aussichten/Museen/Attraktionen sowie besuchenswerte
  //     historische Typen (Burg/Ruine/Kloster/Stadttor …) – ohne den Flut-Effekt
  //     durch Grenzsteine/Gedenktafeln. Alle Zweige gleich streng behandelt.
  const q = `[out:json][timeout:60];`
    + `(nwr["tourism"="attraction"]["name"]["wikidata"](around:${R},${lat},${lon});`
    +  `nwr["tourism"="viewpoint"]["name"]["wikidata"](around:${R},${lat},${lon});`
    +  `nwr["tourism"="museum"]["name"]["wikidata"](around:${R},${lat},${lon});`
    +  `nwr["historic"]["name"]["wikidata"](around:${R},${lat},${lon}););`
    + `out tags center 150;`
    + `(nwr["tourism"="attraction"]["name"](around:${R},${lat},${lon});`
    +  `nwr["tourism"="viewpoint"]["name"](around:${R},${lat},${lon});`
    +  `nwr["tourism"="museum"]["name"](around:${R},${lat},${lon});`
    +  `nwr["historic"~"^(castle|fort|ruins|city_gate|monastery|manor|palace|tower|archaeological_site|aqueduct)$"]["name"](around:${R},${lat},${lon}););`
    + `out tags center 150;`;
...
export default function App() {
  ...
  const [warnZu, setWarnZu] = useState(false); // Sicherungs-Hinweis nur für diese Sitzung ausblendbar
    ...
    <div className="min-h-screen bg-stone-50 text-stone-800">
      ...
      <div className="safe-top mx-auto w-full max-w-2xl px-4 pb-24 pt-6">
        ...
        {activeTrip ? (
          ...
          <>
            <AppMenu
              homeTab={homeTab} setHomeTab={setHomeTab} toolKey={toolKey} setToolKey={setToolKey}
              titel={<span className="inline-flex shrink-0 items-center gap-1.5"><Compass className="h-4 w-4 text-emerald-700 dark:text-emerald-300" /><span className="hidden text-sm font-bold tracking-tight text-stone-900 sm:inline dark:text-stone-50">Urlaubsplaner</span></span>}
              rechts={<ThemeToggle />}
            />
            {!warnZu && homeTab !== "reisen" && (tageSeitSicherung() === null || tageSeitSicherung() >= WARN_TAGE) && (
              <div className="mb-3 flex items-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-800">
                <ShieldAlert className="h-4 w-4 shrink-0" />
                <button onClick={() => setHomeTab("reisen")} className="min-w-0 flex-1 truncate text-left">
                  {tageSeitSicherung() === null ? "Nicht gesichert – nur auf diesem Gerät" : `Sicherung vor ${tageSeitSicherung()} Tagen`} · jetzt sichern ›
                </button>
                <button onClick={() => setWarnZu(true)} aria-label="Hinweis ausblenden" className="shrink-0 rounded p-0.5 text-amber-600 hover:text-amber-900"><X className="h-3.5 w-3.5" /></button>
              </div>
            )}
            ...
            {homeTab === "reisen" && (
              <>
                ...
                <TripList trips={data.trips} onOpen={setActiveId} onCreate={() => setNeuOffen(true)} onDelete={deleteTrip} onDuplicate={duplicateTrip} onExportAll={exportAll} onImportAll={importAll} />
...
function DaysView({ trip, dayList, items, onAdd, onAddMany, onRemove, onPatch, onApply, onUpdateDays, onMoveDay, dragId, dropTarget, onDragStart }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        const verortet = items.filter((i) => i.kategorie !== "kosten" && i.lat != null && i.lon != null).length;
        if (dayList.length < 1 || verortet < 2) return null;
        const verteilen = () => {
          const patches = verteileAufTage(items, dayList);
          if (Object.keys(patches).length && window.confirm(`${verortet} verortete Punkte automatisch nach Nähe auf ${dayList.length} Tag(e) verteilen? Die bestehende Tageszuordnung wird dabei neu gesetzt.`)) onApply(patches);
        };
        return (
          <button onClick={verteilen} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800 hover:bg-emerald-100">
            <Route className="h-4 w-4" /> Punkte automatisch auf die Tage verteilen
          </button>
        );
      })()}
