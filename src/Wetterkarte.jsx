    ...
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1 rounded-lg border border-stone-200 bg-white pl-2">
            <Globe className="h-4 w-4 text-stone-400" />
            <select value={land} onChange={(e) => wechsle(e.target.value)} disabled={loading}
              className="rounded-lg bg-white py-2 pr-2 text-sm focus:outline-none disabled:opacity-60">
      ...
      {data && proj && (
        <>
          ...
          <div className="mt-1 text-center text-sm font-semibold text-stone-800">{data.land} · {label} <span className="text-xs font-normal text-stone-400">(Tag {tag + 1}/{data.dates.length})</span></div>
          ...
          <div className="mt-2 flex items-start gap-2 text-xs text-stone-400"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Quelle: Open-Meteo (frei, ohne Schlüssel) · Städte-Übersicht, Prognose ohne Garantie{data.land === "Deutschland" ? " · Ring = Celle" : ""}.</span></div>
        </>
      )}
