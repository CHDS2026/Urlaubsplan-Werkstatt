...
import { Plane, Train, Search, ExternalLink, Globe2, Sparkles, Info, CalendarDays, Sun, Tag, Check, X, Luggage, ChevronRight, ChevronLeft } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
        ...
        <span>Grün = <strong>kein Bundesland hat Schulferien</strong> (erfahrungsgemäß günstiger), rot = viele Bundesländer in Ferien. Tag antippen öffnet die Suche.</span>
      </div>
      ...
      <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {tage.map((t) => {
            ...
            const titel = vorbei ? "vergangen" : reise ? `Reise: ${reise.name || "Ohne Titel"}` : ft ? ft : n ? `${n} Bundesländer in Ferien` : "keine Schulferien";
            return (
              <button key={t} onClick={() => oeffne(t)} disabled={vorbei}
                ...
                title={titel}
                className={`relative flex h-14 flex-col items-center justify-center rounded-lg border text-xs transition ${farbe(p, t)} ${reise && !vorbei ? "border-sky-400" : we && !vorbei ? "border-emerald-300" : "border-stone-200"} ${vorbei ? "" : "hover:border-emerald-400"}`}>
