...
function CelleTouren({ art, onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  return (
    <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 dark:border-emerald-800 dark:bg-emerald-950/40">
      ...
      <div className="mt-3">
        <Wegkarte spots={mapSpots} selectedId={sel} onSelect={(id) => { setSel(sel === id ? null : id); setMenuFor(null); }} name={`${liste.length} Touren · Landkreis Celle`} hoehe="300px" />
      </div>
        ...
        {liste.map((t) => {
          ...
          return (
            <div key={t.key} className={"rounded-lg px-2.5 py-2 text-sm transition " + (aktiv ? "bg-white ring-1 ring-emerald-300 dark:bg-stone-900 dark:ring-emerald-700" : "bg-white/70 dark:bg-stone-900/60")}>
              <div className="flex items-center gap-2">
                <button onClick={() => { setSel(aktiv ? null : t.key); setMenuFor(null); }} className="min-w-0 flex-1 text-left">
                  <span className="flex flex-wrap items-center gap-1.5">
              ...
              {aktiv && (
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-stone-500 dark:text-stone-400">
                  <span>Start: {t.area}. Kartenpunkt = Ausgangsort, nicht der Verlauf.</span>
          ...
          {data.liste.map((r) => {
            ...
            return (
              <div key={r.id} className={"rounded-lg px-2.5 py-2 text-sm transition " + (aktiv ? "bg-emerald-50 ring-1 ring-emerald-300 dark:bg-emerald-950 dark:ring-emerald-700" : "bg-stone-50 dark:bg-stone-800")}>
                <div className="flex items-center gap-2">
                  <button onClick={() => setSel(aktiv ? null : r.id)} className="min-w-0 flex-1 text-left">
                    <span className="flex flex-wrap items-center gap-1.5">
