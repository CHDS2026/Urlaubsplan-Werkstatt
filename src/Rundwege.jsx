...
export default function Rundwege({ onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  const sug = (w) => { const d = eck(w), sp = spots[w.key]; return { name: w.n, gebiet: w.r + ", " + w.bl, info: infoText(w), lat: d && d.lat != null ? d.lat : (sp ? sp.lat : null), lon: d && d.lon != null ? d.lon : (sp ? sp.lon : null), kategorie: "wanderung" }; };
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200">
      ...
      <div className="mt-3">
        <Wegkarte
          geojson={fc}
          spots={treffer.map((w) => ({ id: w.key, name: w.n, lat: spots[w.key] ? spots[w.key].lat : null, lon: spots[w.key] ? spots[w.key].lon : null })).filter((p) => p.lat != null)}
          selectedId={sel}
