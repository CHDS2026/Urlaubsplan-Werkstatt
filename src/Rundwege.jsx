...
/* ══════════════ Gipfel: leichte Gipfeltouren ohne Steigeisen ══════════════
   ...
   Wie gesucht wird – zwei kleine Abfragen, KEINE Geometrie, deshalb schnell:
     ...
     2. davon die, die ein Weg mit sac_scale ≤ T2 bzw. = T3 berührt -> Trefferliste
   Die schwere Arbeit macht Overpass; im Browser landen nur die Gipfel selbst.
...
async function ladeGipfel(term, radiusKm) {
  ...
  const qA = `[out:json][timeout:90];${alle}.alle out count;way["sac_scale"~"^(${SAC_LEICHT})$"](${um})->.w;node.alle(around.w:100);out body ${MAX_GIPFEL};`;
  const qB = `[out:json][timeout:90];${alle}way["sac_scale"="demanding_mountain_hiking"](${um})->.w;node.alle(around.w:100);out body ${MAX_GIPFEL};`;
...
export default function Rundwege({ onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200">
      ...
      {art === "gipfel" ? <GipfelSuche onAdd={onAdd} onCreateTrip={onCreateTrip} trips={trips} onAddToTrip={onAddToTrip} /> : (<>
