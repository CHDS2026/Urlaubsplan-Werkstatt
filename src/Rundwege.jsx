...
export default function Rundwege({ onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  async function waehle(w) {
    ...
    if (neu && w.art === "rad" && !geo[w.id] && !geoBusy) {
      setGeoBusy(w.id);
      try {
        const ls = await ladeGeomEinzeln(w.id);            // erst warten …
        setGeo((o) => ({ ...o, [w.id]: ls }));             // … dann setzen (Callback ist nicht async!)
  ...
  /* Karte: Fuß = die gefilterten Linien (dadurch zoomt sie auf Bundesland/Suche);
     Rad = der geladene, ausgewählte Verlauf.
     WICHTIG mit useMemo: sonst entstünde bei jedem Render ein neues Objekt und die
     Karte würde endlos neu zoomen – man könnte sie nicht verschieben. */
  const trefferIds = treffer.map((w) => w.id).join(",");
  const fc = useMemo(() => {
    if (art === "fuss" && data) {
      const ids = new Set(trefferIds ? trefferIds.split(",").map(Number) : []);
      const features = data.fc.features.filter((f) => ids.has(f.properties.id));
      return features.length ? { type: "FeatureCollection", features } : null;
    }
    if (art === "rad" && sel) {
      const w = alle.find((x) => x.id === sel), ls = w && linienVonWeg(w);
      if (ls) return { type: "FeatureCollection", features: [{ type: "Feature", properties: { id: w.id, name: w.name }, geometry: { type: "MultiLineString", coordinates: ls.map((g) => g.map((p) => [p.lon, p.lat])) } }] };
    }
    return null;
    // eslint-disable-next-line
  }, [art, data, trefferIds, sel, geo[sel]]);
