    ...
    if (e.type === "relation" && Array.isArray(e.members)) {
      for (const m of e.members) {
        if (m.type === "way" && Array.isArray(m.geometry) && m.geometry.length > 1) {
          feats.push({ type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: m.geometry.map((p) => [p.lon, p.lat]) } });
        }
      }
    } else if (e.type === "way" && Array.isArray(e.geometry) && e.geometry.length > 1) {
      feats.push({ type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: e.geometry.map((p) => [p.lon, p.lat]) } });
    }
  }
