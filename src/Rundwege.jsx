...
import { jget as jgetBasis } from "./lib/net.js";
import { abstandKm } from "./lib/abstand.js";
const jget = (url, ms = 20000) => jgetBasis(url, ms); // längeres Zeitlimit für Overpass
...
const zeilenId = (p, k) => p + String(k).replace(/[^a-zA-Z0-9]+/g, "-");
function scrollZuZeile(id) {
  if (typeof document === "undefined" || typeof requestAnimationFrame === "undefined") return;
  requestAnimationFrame(() => { const el = document.getElementById(id); if (el && el.scrollIntoView) el.scrollIntoView({ behavior: "smooth", block: "center" }); });
}
...
/* Stabiler, same-origin Cache-Schlüssel aus der Abfrage (damit die Cache-API ihn annimmt). */
function overpassKey(q) { let h = 5381; for (let i = 0; i < q.length; i++) h = ((h << 5) + h + q.charCodeAt(i)) | 0; return "/__ovp__/" + (h >>> 0); }
async function overpass(q, ms = 120000) {
  let letzter = null;
  for (const url of OVERPASS) {
    try {
      const res = await jpost(url, q, ms);
      /* Erfolg zwischenspeichern: Overpass läuft per POST und wird vom Service Worker NICHT
         gecacht – so sind zuletzt geladene Routen/Gipfel auch offline wieder da. */
      try {
        if (typeof caches !== "undefined") {
          const c = await caches.open("api-overpass");
          await c.put(overpassKey(q), new Response(JSON.stringify(res), { headers: { "content-type": "application/json" } }));
          const keys = await c.keys(); if (keys.length > 60) await c.delete(keys[0]); // FIFO-Deckel
        }
      } catch (e) {}
      return res;
    } catch (e) { letzter = e; }
  }
  /* Alle Server nicht erreichbar (offline?) -> zuletzt gecachtes Ergebnis DIESER Abfrage. */
  try {
    if (typeof caches !== "undefined") {
      const c = await caches.open("api-overpass");
      const hit = await c.match(overpassKey(q));
...
const laengeVon = (ls) => { let s = 0; for (const g of ls) for (let i = 1; i < g.length; i++) s += abstandKm(g[i - 1].lat, g[i - 1].lon, g[i].lat, g[i].lon); return s; };
const rundVon = (ls) => { if (ls.length < 2) return false; const a = ls[0][0], g = ls[ls.length - 1], b = g[g.length - 1]; return abstandKm(a.lat, a.lon, b.lat, b.lon) < 1; };
...
async function ladeRoute(w, center) {
  const typ = w.art === "rad" ? "bicycle" : "hiking";

  // 1) SCHNELLWEG: exakter Name ist in Overpass indexiert -> deutschlandweit, ohne Box-Scan.
  //    Trifft für die meisten berühmten Wege sofort (Name = OSM-Name).
  try {
    const oe = await overpass(`[out:json][timeout:45];rel["route"="${typ}"]["name"="${String(w.n).replace(/"/g, '\\"')}"];out geom;`, 45000);
    const rel = ((oe && oe.elements) || []).find((e) => e.type === "relation" && e.tags && linienVon(e).length && inDeutschland((linienVon(e)[0][0] || {}).lat, (linienVon(e)[0][0] || {}).lon));
    if (rel) return baueRoute(rel);
  } catch (e) { /* Schnellweg fehlgeschlagen -> Fallback */ }

  // 2) FALLBACK: Namensregex in einer Box um die Region (kleinere Box = schneller).
  const c = center || await orten(w.r);
  if (!c) throw new Error("Region nicht gefunden");
  const d = w.art === "rad" ? 2.2 : 0.5;
  const box = [(c.lat - d).toFixed(3), (c.lon - d * 1.5).toFixed(3), (c.lat + d).toFixed(3), (c.lon + d * 1.5).toFixed(3)].join(",");
  const o = await overpass(`[out:json][timeout:60];rel["route"="${typ}"]["name"~"${w.q}",i](${box});out geom;`, 60000);
  const re = new RegExp(w.q, "i");
  const treffer = ((o && o.elements) || []).filter((e) => e.type === "relation" && e.tags && e.tags.name && re.test(e.tags.name) && linienVon(e).length);
  if (!treffer.length) throw new Error("in OpenStreetMap nicht gefunden");
  treffer.sort((a, b) => a.tags.name.length - b.tags.name.length); // kürzester Name = spezifischste Übereinstimmung
  return baueRoute(treffer[0]);
}
...
async function ladeGipfel(term, radiusKm) {
  ...
  ].map((p) => ({ ...p, weg: Math.round(abstandKm(c.lat, c.lon, p.lat, p.lon) * 10) / 10 }));
...
const BERGE = [
  ...
  { rg: "Tirol", n: "Neunerköpfle", geo: "Neunerköpfle, Tannheim, Tirol", area: "Tannheimer Tal · Außerfern", grad: "leicht", bahn: true },
  { rg: "Tirol", n: "Füssener Jöchle (Gamskopf)", geo: "Füssener Jöchle, Grän, Tirol", area: "Tannheimer Tal · Außerfern", grad: "leicht", bahn: true },
  { rg: "Tirol", n: "Krinnenspitze", geo: "Krinnenspitze, Nesselwängle, Tirol", area: "Tannheimer Tal · Außerfern", grad: "mittel", bahn: false },
  { rg: "Tirol", n: "Einstein", geo: "Einstein, Tannheim, Tirol", area: "Tannheimer Tal · Außerfern", grad: "mittel", bahn: false },

  /* --- Kleinwalsertal (Vorarlberg – über Oberstdorf erreichbar; unter Tirol/Österreich gruppiert) --- */
  { rg: "Tirol", n: "Kanzelwand", geo: "Kanzelwand, Riezlern, Kleinwalsertal", area: "Kleinwalsertal · Vorarlberg", grad: "leicht", bahn: true },
  { rg: "Tirol", n: "Walmendingerhorn", geo: "Walmendingerhorn, Mittelberg, Kleinwalsertal", area: "Kleinwalsertal · Vorarlberg", grad: "leicht", bahn: true, hinweis: "Bergbahn wird bis Winter 2026/27 neu gebaut – Sommerbetrieb 2026 vorab prüfen" },

  /* --- Oberstdorf / Allgäu (Bayern) --- */
  { rg: "Bayern", n: "Nebelhorn", geo: "Nebelhorn, Oberstdorf", area: "Oberstdorf · Allgäuer Alpen", grad: "leicht", bahn: true },
  { rg: "Bayern", n: "Fellhorn", geo: "Fellhorn, Oberstdorf", area: "Oberstdorf · Allgäuer Alpen", grad: "leicht", bahn: true },
  { rg: "Bayern", n: "Söllereck", geo: "Söllereck, Oberstdorf", area: "Oberstdorf · Allgäuer Alpen", grad: "leicht", bahn: true },
];
...
/* Echte Gipfelhöhe LIVE aus OpenStreetMap holen (nächster Gipfelpunkt mit ele-Tag im
   Umkreis) – statt Meterzahlen aus dem Gedächtnis. Gibt Meter oder null zurück. */
async function gipfelHoehe(lat, lon) {
  const o = await overpass(`[out:json][timeout:40];node["natural"="peak"]["ele"](around:800,${lat.toFixed(5)},${lon.toFixed(5)});out tags 12;`, 40000);
  let best = null, bestD = 1e9;
  for (const e of ((o && o.elements) || [])) {
    if (!e.tags || e.tags.ele == null || e.lat == null) continue;
    const h = parseFloat(String(e.tags.ele).replace(",", "."));
    if (!isFinite(h)) continue;
    const d = abstandKm(lat, lon, e.lat, e.lon);
    if (d < bestD) { bestD = d; best = Math.round(h); }
  }
  return best;
}

/* Kurzer, sachlicher Charakter-Satz je Gipfel (Namens-Map – keine Höhen darin, die
   kommt live aus OSM). Warum dieser Berg: Landschaft, markantes Merkmal, Bahn. */
const BERG_INFO = {
  "Herzogstand": "Klassiker über Walchensee und Kochelsee; Sessellift ab Kochel, luftiger Panoramaweg Richtung Heimgarten.",
  "Jochberg": "Aussichtskanzel über dem türkisen Walchensee, familientauglich.",
  "Wank": "Der Sonnenbalkon über Garmisch mit Blick aufs Wettersteingebirge; Bergbahn.",
  "Hoher Kranzberg": "Aussichtsturm über Mittenwald mit weitem Karwendel-Blick; Sessellift.",
  "Wallberg": "Hausberg über dem Tegernsee, beliebt für Sonnenuntergänge; Bergbahn.",
  "Neureuth": "Kurzer Anstieg zum Panorama-Berggasthaus über dem Tegernsee.",
  "Riederstein": "Steiler kurzer Steig zur Felskapelle mit Tiefblick auf den Tegernsee.",
  "Blomberg": "Familienberg über Bad Tölz mit Sommerrodelbahn; Bergbahn.",
  "Brauneck": "Weitläufiger Aussichtsgipfel über Lenggries; Bergbahn.",
  "Seekarkreuz": "Ruhiger Wiesengipfel über Lenggries, schöne Rundtour.",
  "Fockenstein": "Aussichtsgipfel über dem Tegernsee mit Blick bis zum Karwendel.",
  "Hirschberg": "Beliebter Grasgipfel über Kreuth mit weitem Alpenpanorama.",
  "Breitenstein": "Bekannter Aussichtsberg bei Bayrischzell, im Winter oft mit Schneeschuhen.",
  "Wendelstein": "Markanter Gipfel mit Bergkirche und Sternwarte; Zahnradbahn oder Seilbahn.",
  "Hinteres Hörnle": "Sanfter Aussichtsberg der Ammergauer Alpen; Hörnlebahn.",
  "Kranzhorn": "Grenzgipfel über dem Inntal – Gipfelkreuz auf bayerischer und Tiroler Seite.",
  "Heuberg": "Aussichtsgipfel über dem Inntal, kurzer Anstieg von der Kaseralm.",
  "Spitzstein": "Einfacher Chiemgau-Klassiker mit Kaiserblick, Almen am Weg.",
  "Hochries": "Aussichtsberg am Samerberg; Bergbahn, weiter Blick über den Chiemgau.",
  "Kampenwand": "Markante Felszacken über dem Chiemsee; Bergbahn, großes Alpenpanorama.",
  "Laubenstein": "Leichter Chiemgau-Gipfel über der Hochrieshütte.",
  "Geigelstein": "Der Blumenberg des Chiemgaus – artenreiches Naturschutzgebiet.",
  "Hochgern": "Aussichtsgipfel über Marquartstein mit Chiemsee-Blick.",
  "Hochfelln": "Chiemgau-Aussichtsberg mit Gipfelkapelle; Bergbahn ab Bergen.",
  "Grünstein": "Aussichtskanzel über dem Königssee, beliebter Einsteiger-Gipfel.",
  "Grünten": "Der Wächter des Allgäus – markanter Vorberg mit weitem Blick.",
  "Neunerköpfle": "Aussichtsbalkon über dem Tannheimer Tal; Bergbahn, Panorama-Rundweg.",
  "Füssener Jöchle (Gamskopf)": "Familiengipfel über Grän; Bergbahn, kurzer Weg zum Gamskopf.",
  "Krinnenspitze": "Beliebter Aussichtsgipfel über dem Tannheimer Tal.",
  "Einstein": "Markanter Grenzgipfel über Tannheim mit Rundblick.",
  "Kanzelwand": "Grenzgipfel über dem Kleinwalsertal, mit dem Fellhorn verbunden; Bergbahn.",
  "Walmendingerhorn": "Berg der Sinne mit Drei-Länder-Blick über dem Kleinwalsertal; Bergbahn.",
  "Hohe Salve": "Einer der beliebtesten Aussichtsberge Tirols, Rundumblick; Gondel.",
  "Kitzbüheler Horn": "Aussichtsberg über Kitzbühel mit Alpenblumengarten; Bergbahn.",
  "Patscherkofel": "Innsbrucks Hausberg mit dem Zirbenweg; Bahn ab Igls.",
  "Rofanspitze": "Aussichtsgipfel im Rofan über dem Achensee; Seilbahn ab Maurach.",
  "Galtjoch": "Einsteiger-Gipfel im Lechtal mit weitem Panorama.",
  "Simmering": "Aussichtsgipfel am Mieminger Plateau, technisch einfach.",
  "Nockspitze (Saile)": "Markanter Aussichtsgipfel über dem Inntal bei Innsbruck.",
  "Thaneller": "Höchster der westlichen Lechtaler Vorberge, weiter Rundblick.",
  "Tschirgant": "Aussichtsgipfel über dem Inntal bei Imst, konditionell fordernd.",
  "Schwarzkogel": "Ruhiger Aussichtsgipfel in den Kitzbüheler Alpen.",
  "Filzenkogel": "Familiengipfel am Ahorn über Mayrhofen; Ahornbahn.",
  "Nebelhorn": "Gipfel der 400 Berge über Oberstdorf; Bahn, Rundumblick.",
  "Fellhorn": "Der Blumenberg über Oberstdorf; Bahn, Gratweg zur Kanzelwand.",
  "Söllereck": "Familienberg über Oberstdorf; Bahn, sanfte Wege.",
};
...
function BeliebteBerge({ onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  const [hoehe, setHoehe] = useState({});     // key -> Meter | "leer"
  const [hoeheBusy, setHoeheBusy] = useState(null);

  const liste = BERGE.filter((b) => b.rg === rg).map((b) => ({ ...b, key: b.rg + ":" + b.n }));

  /* Antippen: auswählen UND die echte Höhe live aus OSM nachladen (einmal je Gipfel). */
  async function waehle(b) {
    const neu = sel === b.key ? null : b.key;
    setSel(neu); setMenuFor(null);
    if (neu) scrollZuZeile(zeilenId("bb-", b.key));
    if (!neu || hoehe[b.key] !== undefined || hoeheBusy) return;
    const c = spots[b.key];
    if (!c) return;
    setHoeheBusy(b.key);
    try { const h = await gipfelHoehe(c.lat, c.lon); setHoehe((o) => ({ ...o, [b.key]: h != null ? h : "leer" })); }
    catch (e) { setHoehe((o) => ({ ...o, [b.key]: "leer" })); }
    finally { setHoeheBusy(null); }
  }
  ...
  return (
    <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 dark:border-emerald-800 dark:bg-emerald-950/40">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => { setRg("Tirol"); setSel(null); setMenuFor(null); }} className={chip(rg === "Tirol")}>Tirol/Österreich ({BERGE.filter((b) => b.rg === "Tirol").length})</button>
      ...
      <div className="mt-3">
        <Wegkarte
          spots={mapSpots}
          selectedId={sel}
          onSelect={(id) => { const b = liste.find((x) => x.key === id); if (b) waehle(b); }}
      ...
      <div className="mt-3 space-y-1.5">
        {treffer.map((b) => {
          ...
          return (
            <div key={b.key} id={zeilenId("bb-", b.key)} className={"scroll-mt-24 rounded-lg px-2.5 py-2 text-sm transition " + (aktiv ? "bg-white ring-1 ring-emerald-300 dark:bg-stone-900 dark:ring-emerald-700" : "bg-white/70 dark:bg-stone-900/60")}>
              <div className="flex items-center gap-2">
                <button onClick={() => waehle(b)} className="min-w-0 flex-1 text-left">
              ...
              {aktiv && (
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-stone-500 dark:text-stone-400">
                  ...
                  {hoeheBusy === b.key && <span className="inline-flex items-center gap-1"><Loader2 className="h-3 w-3 animate-spin" /> Höhe …</span>}
                  {typeof hoehe[b.key] === "number" && <span className="inline-flex items-center gap-1 font-medium text-stone-600 dark:text-stone-300"><TrendingUp className="h-3 w-3" /> {hoehe[b.key]} m ü. NN</span>}
                  {BERG_INFO[b.n] && <span className="basis-full text-stone-500 dark:text-stone-400">{BERG_INFO[b.n]}</span>}
                  {b.hinweis && <span className="inline-flex items-center gap-1 rounded-md bg-amber-100 px-2 py-0.5 font-medium text-amber-800 dark:bg-amber-950 dark:text-amber-300"><Info className="h-3 w-3" /> {b.hinweis}</span>}
...
/* ══════════════ Region durchsuchen: LIVE-Suche echter OSM-Routen ══════════════

   Warum es das gibt: Die kuratierten Favoriten oben funktionieren nur, wenn genau diese
   Route als benannte OSM-Relation existiert. Für lokale Wege (z. B. im Landkreis Celle /
   Naturpark Südheide) trifft das oft nicht zu – dann bleibt die Favoritenliste leer.
   Diese Suche fragt Overpass stattdessen nach ALLEN benannten Wander- bzw. Radrouten
   (route=hiking / route=bicycle) im Umkreis einer Region ab und zeigt, was wirklich da
   ist. Robust gegen das, was OSM enthält – für Celle genauso wie für jede andere Gegend.

   EHRLICH:
    • Gezeigt wird, was in OpenStreetMap als benannte Route eingetragen ist – nicht mehr
      und nicht weniger. Fehlt ein Weg, fehlt er in OSM (nichts erfunden).
    • Länge nur, wenn sie an der Relation getaggt ist (distance-Tag). Sonst keine Zahl.
    • Der Kartenpunkt ist die Mitte der Route (out center), nicht der Startpunkt. */
const NETZ = {
  iwn: { t: "international", r: 0 }, nwn: { t: "national", r: 1 },
  rwn: { t: "regional", r: 2 }, lwn: { t: "lokal", r: 3 },
  icn: { t: "international", r: 0 }, ncn: { t: "national", r: 1 },
  rcn: { t: "regional", r: 2 }, lcn: { t: "lokal", r: 3 },
};

function parseDistKm(v) {
  ...
  if (/m\b/.test(s) && !/km/.test(s) && n > 400) n = n / 1000; // Meter -> km
  return Math.round(n * 10) / 10;
}

async function ladeRegionRouten(term, radiusKm, art) {
  const c = await ortenFrei(term);
  if (!c) throw new Error("Region nicht gefunden");
  const typ = art === "rad" ? "bicycle" : "hiking";
  const q = `[out:json][timeout:60];rel["route"="${typ}"]["name"](around:${Math.round(radiusKm * 1000)},${c.lat.toFixed(5)},${c.lon.toFixed(5)});out tags center 90;`;
  const o = await overpass(q, 60000);
  const seen = new Set();
  const liste = [];
  for (const e of ((o && o.elements) || [])) {
    if (e.type !== "relation" || !e.tags || !e.tags.name) continue;
    const ce = e.center || {};
    if (ce.lat == null || ce.lon == null) continue;
    const key = e.tags.name.toLowerCase();
    if (seen.has(key)) continue;
    seen.add(key);
    const netz = NETZ[e.tags.network] || null;
    liste.push({
      id: e.id, name: e.tags.name, ref: e.tags.ref || "",
      netz: netz ? netz.t : "", rang: netz ? netz.r : 4,
      km: parseDistKm(e.tags.distance), lat: ce.lat, lon: ce.lon,
      weg: Math.round(abstandKm(c.lat, c.lon, ce.lat, ce.lon) * 10) / 10,
    });
  }
  liste.sort((a, b) => a.rang - b.rang || a.weg - b.weg || a.name.localeCompare(b.name));
  return { ort: c.name, lat: c.lat, lon: c.lon, liste, voll: liste.length >= 90 };
}

/* ══════════════ Rund um Celle: kuratierte lokale Touren ══════════════

   Warum eigen und PUNKT-basiert: Die kurzen Touren im Celler Land / Naturpark Südheide
   sind in OpenStreetMap oft NICHT als benannte Route erfasst – Favoriten und Live-Suche
   finden sie deshalb nicht. Diese Liste umgeht das: echte, belegte Touren als verortete
   Startpunkte (wie die Gipfel), unabhängig davon, ob OSM eine Route dafür hat.

   Quellen: celle-tourismus.de (offiziell), Naturpark Südheide / region-celle-navigator.de
   (10 ausgezeichnete Rad-Rundtouren, fahrradfreundlichste Kommune 2014), komoot, GPS-
   Wanderatlas. Längen nur, wo belegt; für die Naturpark-Radtouren gilt die dokumentierte
   Spanne 20–60 km statt einer erfundenen Einzelzahl.

   EHRLICH: Der Kartenpunkt ist der START/Ausgangsort, nicht der Routenverlauf. Den
   vollständigen Verlauf mit GPX gibt es auf region-celle-navigator.de bzw. celle-
   tourismus.de – dorthin führt der Link. Verortung live über Photon. */
const CELLE_TOUREN = [
  // --- Zu Fuß ---
  { art: "fuss", n: "Hüttensee-Rundweg", geo: "Meißendorf, Winsen Aller", area: "Meißendorfer Teiche", km: 4.9, grad: "leicht" },
  { art: "fuss", n: "Altstadt-Rundweg Celle", geo: "Schloss Celle", area: "Celle · Altstadt", km: 8, grad: "leicht" },
  { art: "fuss", n: "Wo der Wald mit der Heide kusselt (kurze Runde)", geo: "Müden Örtze", area: "Müden · Südheide", km: 5.1, grad: "leicht" },
  { art: "fuss", n: "Wietzer Berg (Heideschleife Müden)", geo: "Müden Örtze", area: "Müden · Südheide", km: 12.5, grad: "mittel" },
  { art: "fuss", n: "Oberoher Heide – Weesen", geo: "Oberohe, Faßberg", area: "Faßberg · Südheide", km: 12, grad: "mittel" },
  { art: "fuss", n: "Becklinger Moor", geo: "Becklingen, Bergen", area: "Bergen · Südheide", km: 15, grad: "mittel" },
  { art: "fuss", n: "Lutter-Runde bei Bargfeld", geo: "Bargfeld, Eldingen", area: "Luttertal · Eldingen", km: 9.4, grad: "mittel" },
  // --- Fahrrad ---
  { art: "rad", n: "Mit dem Fahrrad durch vier Jahrhunderte", geo: "Schloss Celle", area: "Celle · Stadtrunde", km: 5, grad: "leicht" },
  { art: "rad", n: "Fachwerk, Backsteingotik & Bauhaus", geo: "Celle", area: "Celle · Radthementour", spanne: "20–60 km", grad: "leicht" },
  { art: "rad", n: "Land der nickenden Pferde", geo: "Wathlingen", area: "Wathlingen · Radthementour", spanne: "20–60 km", grad: "leicht" },
  { art: "rad", n: "Malerische Heidehöfe & Orte der Erinnerung", geo: "Bergen, Celle", area: "Bergen · Radthementour", spanne: "20–60 km", grad: "leicht" },
  { art: "rad", n: "Von der Moorlandschaft zur Pferderegion", geo: "Adelheidsdorf", area: "Adelheidsdorf · Radthementour", spanne: "20–60 km", grad: "leicht" },
];

/* ── Verlauf (Route-Geometrie) aus OpenStreetMap laden und als GeoJSON-Linien liefern ──
   Damit die Karte den WEG zeichnet, nicht nur die Nadel. Zwei Wege:
     • per Relations-ID  (Live-Suche – dort sind es echte OSM-Relationen)
     • per Namensuche nahe dem Startpunkt (kuratierte Celle-Touren – wo OSM sie hat)
   Gibt null zurueck, wenn OSM keine passende Route hat -> dann wird NICHT gezeichnet
   (keine erfundene Linie), sondern Startpunkt + GPX-Link bleiben. */
function geomAusOSM(o) {
  const lines = [];
  for (const e of ((o && o.elements) || [])) {
    if (e.type === "relation" && Array.isArray(e.members)) {
      for (const m of e.members) {
        if (m.type === "way" && Array.isArray(m.geometry) && m.geometry.length > 1) lines.push(m.geometry.map((p) => [p.lon, p.lat]));
      }
    } else if (e.type === "way" && Array.isArray(e.geometry) && e.geometry.length > 1) {
      lines.push(e.geometry.map((p) => [p.lon, p.lat]));
    }
  }
  // MultiLineString – gleiches Format wie die Favoriten (dort läuft das Zeichnen sauber)
  return lines.length ? { type: "FeatureCollection", features: [{ type: "Feature", properties: {}, geometry: { type: "MultiLineString", coordinates: lines } }] } : null;
}

async function verlaufNachId(relId) {
  const o = await overpass(`[out:json][timeout:60];rel(${relId});out geom;`, 60000);
  return geomAusOSM(o);
}

function kernwort(name) {
  const stop = new Set(["rundweg", "rundwanderweg", "runde", "kurze", "kurz", "der", "die", "das", "mit", "und", "am", "im", "heideschleife", "weg", "tour", "celle", "landkreis", "ueber", "\u00fcber"]);
  const worte = name.replace(/[()\-\u2013,.]/g, " ").split(/\s+/).filter(Boolean).filter((w) => !stop.has(w.toLowerCase()) && w.length >= 4);
  worte.sort((a, b) => b.length - a.length);
  return (worte[0] || name.split(/\s+/)[0] || name).replace(/["\\]/g, "");
}

async function verlaufNachName(name, lat, lon, art) {
  const typ = art === "rad" ? "bicycle" : "hiking";
  const kern = kernwort(name);
  const o = await overpass(`[out:json][timeout:60];rel["route"="${typ}"]["name"~"${kern}",i](around:7000,${lat.toFixed(5)},${lon.toFixed(5)});out geom 2;`, 60000);
  return geomAusOSM(o);
}

/* Kurzer, sachlicher Charakter-Satz je Celle-Tour (Namens-Map). Was macht die Runde aus –
   Landschaft, markantes Merkmal. Belegt über celle-tourismus.de / Naturpark Südheide. */
const CELLE_INFO = {
  "Hüttensee-Rundweg": "Kurze Runde um den Hüttensee in den Meißendorfer Teichen – Wasser, Wald und viele Vögel.",
  "Altstadt-Rundweg Celle": "Durch die Fachwerk-Altstadt mit über 400 Fachwerkhäusern, Schloss und Französischem Garten.",
  "Wo der Wald mit der Heide kusselt (kurze Runde)": "Kurze Heide-Wald-Runde bei Müden, im Spätsommer lila blühende Heide.",
  "Wietzer Berg (Heideschleife Müden)": "Zertifizierte Heideschleife über den Wietzer Berg mit Blick über die Heideflächen.",
  "Oberoher Heide – Weesen": "Weite, stille Heidelandschaft der Südheide zwischen Oberohe und Weesen.",
  "Becklinger Moor": "Runde durch Heide und das Becklinger Moor bei Bergen, teils auf Bohlenwegen.",
  "Lutter-Runde bei Bargfeld": "Am klaren Lutterlauf entlang, Wald und kleine Badeseen bei Bargfeld.",
  "Mit dem Fahrrad durch vier Jahrhunderte": "Kompakte Stadtrunde vorbei an Fachwerk, Schloss und Bauhaus-Bauten.",
  "Fachwerk, Backsteingotik & Bauhaus": "Themen-Radtour durch die Baustile der Region rund um Celle.",
  "Land der nickenden Pferde": "Radtour durch die Pferderegion mit Erdöl-Geschichte um Wathlingen.",
  "Malerische Heidehöfe & Orte der Erinnerung": "Radtour zu Heidehöfen und Gedenkorten rund um Bergen.",
  "Von der Moorlandschaft zur Pferderegion": "Radtour von den Mooren zur Pferderegion um Adelheidsdorf.",
};

function CelleTouren({ art, onAdd, onCreateTrip, trips, onAddToTrip }) {
  const [sel, setSel] = useState(null);
  ...
  const [geomInfo, setGeomInfo] = useState(null); // { key, status: "laedt"|"da"|"leer"|"kein" }
  const ladeRef = useRef(0);

  const liste = CELLE_TOUREN.filter((t) => t.art === art).map((t) => ({ ...t, key: t.art + ":" + t.n }));

  /* Antippen: Startpunkt hervorheben UND versuchen, den echten Verlauf aus OSM zu laden. */
  async function waehle(t) {
    const neu = sel === t.key ? null : t.key;
    setSel(neu); setMenuFor(null); setGeom(null); setGeomInfo(null);
    if (neu) scrollZuZeile(zeilenId("ct-", t.key));
    const myId = ++ladeRef.current;
    if (!neu) return;
    const c = spots[t.key];
    if (!c) { setGeomInfo({ key: t.key, status: "kein" }); return; }
    setGeomInfo({ key: t.key, status: "laedt" });
    try {
      const g = await verlaufNachName(t.n, c.lat, c.lon, art);
      if (ladeRef.current !== myId) return;
      if (g) { setGeom(g); setGeomInfo({ key: t.key, status: "da" }); }
      else setGeomInfo({ key: t.key, status: "leer" });
    } catch (e) { if (ladeRef.current === myId) setGeomInfo({ key: t.key, status: "leer" }); }
  }
  const kat = rad ? "radtour" : "wanderung";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const t of liste) {
        if (!alive) return;
        if (spots[t.key]) continue;
        try { const c = await ortenFrei(t.geo); if (alive && c) setSpots((o) => ({ ...o, [t.key]: { lat: c.lat, lon: c.lon } })); }
        catch (e) { /* Punkt fehlt dann */ }
  ...
  const mapSpots = liste.filter((t) => spots[t.key]).map((t) => ({ id: t.key, name: t.n, kategorie: kat, lat: spots[t.key].lat, lon: spots[t.key].lon }));
  const laenge = (t) => (t.km != null ? t.km + " km" : (t.spanne || ""));
  const infoText = (t) => [rad ? "Radtour" : "Wanderung", t.grad, t.area, laenge(t)].filter(Boolean).join(" · ");
  const sug = (t) => { const c = spots[t.key]; return { name: t.n, gebiet: t.area, info: infoText(t), lat: c ? c.lat : null, lon: c ? c.lon : null, kategorie: kat }; };
  const merke = (t, m) => { setDone((o) => ({ ...o, [t.key]: m })); setMenuFor(null); };
  const nurIdeen = !!onAdd && !onCreateTrip && !(onAddToTrip && trips && trips.length);
  const hatAktion = !!(onAdd || onCreateTrip || (onAddToTrip && trips && trips.length));
  const gStil = { leicht: "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300", mittel: "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300" };
  const zeile = "flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm text-stone-700 transition hover:bg-emerald-50 dark:text-stone-200 dark:hover:bg-stone-800";
  ...
  return (
    <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 dark:border-emerald-800 dark:bg-emerald-950/40">
      <div className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-stone-800 dark:text-stone-100">
        {rad ? <Bike className="h-4 w-4 text-emerald-700 dark:text-emerald-300" /> : <Footprints className="h-4 w-4 text-emerald-700 dark:text-emerald-300" />} Rund um Celle
        {busy && <span className="inline-flex items-center gap-1 text-xs font-normal text-stone-400"><Loader2 className="h-3 w-3 animate-spin" /> verorten …</span>}
      </div>
      <p className="mt-1 text-xs text-stone-500 dark:text-stone-400">Kurze, belegte {rad ? "Rad-Rundtouren" : "Wanderungen"} im Landkreis Celle / Naturpark Südheide – als Startpunkt auf der Karte, unabhängig von OpenStreetMap. Voller Verlauf mit GPX auf region-celle-navigator.de.</p>

      <div className="mt-3">
        <Wegkarte geojson={geom} spots={mapSpots} selectedId={sel} onSelect={(id) => { const t = liste.find((x) => x.key === id); if (t) waehle(t); }} name={`${liste.length} Touren · Landkreis Celle`} hoehe="300px" />
      </div>

      <div className="mt-3 space-y-1.5">
        {liste.map((t) => {
          const aktiv = sel === t.key;
          return (
            <div key={t.key} id={zeilenId("ct-", t.key)} className={"scroll-mt-24 rounded-lg px-2.5 py-2 text-sm transition " + (aktiv ? "bg-white ring-1 ring-emerald-300 dark:bg-stone-900 dark:ring-emerald-700" : "bg-white/70 dark:bg-stone-900/60")}>
              <div className="flex items-center gap-2">
                <button onClick={() => waehle(t)} className="min-w-0 flex-1 text-left">
                  <span className="flex flex-wrap items-center gap-1.5">
                    <span className="font-semibold text-stone-800 dark:text-stone-100">{t.n}</span>
                    <span className={"rounded-full px-2 py-0.5 text-xs font-medium " + gStil[t.grad]}>{t.grad}</span>
                    {laenge(t) && <span className="rounded-full bg-stone-200 px-2 py-0.5 text-xs font-medium text-stone-600 dark:bg-stone-700 dark:text-stone-300">{laenge(t)}</span>}
                  </span>
                  <span className="mt-0.5 flex flex-wrap items-center gap-x-3 text-xs text-stone-500 dark:text-stone-400"><span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {t.area}</span></span>
                </button>
                {done[t.key]
                  ? <span className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"><Check className="h-3.5 w-3.5" /> {done[t.key]}</span>
                  : hatAktion && <button onClick={() => (nurIdeen ? (onAdd(sug(t)), merke(t, "in Ideen")) : setMenuFor(menuFor === t.key ? null : t.key))} className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-700 px-2.5 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-800">{menuFor === t.key ? <X className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />} {nurIdeen ? "Hinzufügen" : "Übernehmen"}</button>}
              </div>

              {aktiv && (
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-stone-500 dark:text-stone-400">
                  {geomInfo && geomInfo.key === t.key && geomInfo.status === "laedt" && <span className="inline-flex items-center gap-1"><Loader2 className="h-3 w-3 animate-spin" /> Verlauf wird aus OpenStreetMap geladen …</span>}
                  {geomInfo && geomInfo.key === t.key && geomInfo.status === "da" && <span className="inline-flex items-center gap-1 text-emerald-700 dark:text-emerald-300"><RouteIcon className="h-3 w-3" /> Verlauf aus OpenStreetMap gezeichnet.</span>}
                  {geomInfo && geomInfo.key === t.key && (geomInfo.status === "leer" || geomInfo.status === "kein") && <span>Verlauf nicht in OpenStreetMap – Startpunkt gesetzt, vollständige Route über den GPX-Link.</span>}
                  {(!geomInfo || geomInfo.key !== t.key) && <span>Start: {t.area}. Kartenpunkt = Ausgangsort.</span>}
                  {CELLE_INFO[t.n] && <span className="basis-full text-stone-500 dark:text-stone-400">{CELLE_INFO[t.n]}</span>}
                  <a href="https://www.region-celle-navigator.de" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-stone-400 transition hover:text-emerald-700 dark:text-stone-500">Verlauf & GPX <ExternalLink className="h-3 w-3" /></a>
                </div>
              )}
                ...
                <div className="mt-2 space-y-1 rounded-lg border border-stone-200 bg-white p-2 dark:border-stone-700 dark:bg-stone-900">
                  {onAddToTrip && trips && trips.length > 0 && (<>
                    ...
                    {trips.slice(0, 8).map((tr) => (
                      <button key={tr.id} onClick={() => { onAddToTrip(tr.id, sug(t)); merke(t, "zu " + (tr.name || "Reise")); }} className={zeile}>
                        <MapPin className="h-3.5 w-3.5 shrink-0 text-emerald-700 dark:text-emerald-300" /> <span className="min-w-0 truncate">{tr.name || "Reise"}</span>
                      </button>
                    ))}
                  </>)}
                  {onCreateTrip && <button onClick={() => { onCreateTrip({ name: t.n, gebiet: t.area, info: infoText(t), anreiseart: "auto", von: "Celle", nach: t.geo, items: [sug(t)] }); merke(t, "neue Reise"); }} className={zeile + " font-medium"}><CalendarPlus className="h-3.5 w-3.5 shrink-0 text-emerald-700 dark:text-emerald-300" /> Neue Reise daraus erstellen</button>}
                  {onAdd && <button onClick={() => { onAdd(sug(t)); merke(t, "in Ideen"); }} className={zeile}><Plus className="h-3.5 w-3.5 shrink-0 text-emerald-700 dark:text-emerald-300" /> In Ideen merken</button>}
                </div>
              )}
            </div>
          );
      ...
      <div className="mt-3 flex items-start gap-2 text-xs text-stone-400 dark:text-stone-500"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>
        Belegt über celle-tourismus.de und den Naturpark Südheide (region-celle-navigator.de; die 10 Rad-Rundtouren sind ADFC-ausgezeichnet). Länge nur, wo angegeben; die Naturpark-Radtouren liegen dokumentiert bei 20–60 km. Der Kartenpunkt ist der Startort, nicht der Verlauf – vollständige Route mit GPX über den Link. Ohne Gewähr.
      </span></div>
    </div>
  );
}

function RegionRoutenSuche({ art, onAdd, onCreateTrip, trips, onAddToTrip }) {
  const [input, setInput] = useState("");
  ...
  const rad = art === "rad";
  const [geom, setGeom] = useState(null);
  const [geomInfo, setGeomInfo] = useState(null); // { id, status }
  const ladeRef = useRef(0);

  /* Antippen einer gefundenen Route: echten Verlauf der OSM-Relation laden und zeichnen. */
  async function waehle(r) {
    const neu = sel === r.id ? null : r.id;
    setSel(neu); setMenuFor(null); setGeom(null); setGeomInfo(null);
    if (neu) scrollZuZeile(zeilenId("rrs-", r.id));
    const myId = ++ladeRef.current;
    if (!neu) return;
    setGeomInfo({ id: r.id, status: "laedt" });
    try {
      const g = await verlaufNachId(r.id);
      if (ladeRef.current !== myId) return;
      if (g) { setGeom(g); setGeomInfo({ id: r.id, status: "da" }); }
      else setGeomInfo({ id: r.id, status: "leer" });
    } catch (e) { if (ladeRef.current === myId) setGeomInfo({ id: r.id, status: "leer" }); }
  }

  async function run(q0, r0) {
    const term = (q0 != null ? q0 : input).trim();
    if (!term || loading) return;
    setLoading(true); setErr(""); setData(null); setSel(null); setMenuFor(null);
    try { setData(await ladeRegionRouten(term, r0 != null ? r0 : radius, art)); }
    catch (e) { setErr((e.message || String(e)) + " – Overpass ist ein freier Dienst und zeitweise überlastet; kurz später noch einmal versuchen."); }
    finally { setLoading(false); }
  }

  const kat = rad ? "radtour" : "wanderung";
  const infoText = (r) => [rad ? "Radroute" : "Wanderroute", r.netz, r.km ? r.km + " km" : "", r.ref ? "Nr. " + r.ref : ""].filter(Boolean).join(" · ");
  const sug = (r) => ({ name: r.name, gebiet: (data && data.ort) || "", info: infoText(r), lat: r.lat, lon: r.lon, kategorie: kat });
  const merke = (r, t) => { setDone((o) => ({ ...o, [r.id]: t })); setMenuFor(null); };
  const nurIdeen = !!onAdd && !onCreateTrip && !(onAddToTrip && trips && trips.length);
  const hatAktion = !!(onAdd || onCreateTrip || (onAddToTrip && trips && trips.length));
  const chip = (on) => "rounded-full px-3 py-1.5 text-xs font-medium transition " + (on ? "bg-emerald-700 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200 dark:bg-stone-800 dark:text-stone-300");
  const zeile = "flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm text-stone-700 transition hover:bg-emerald-50 dark:text-stone-200 dark:hover:bg-stone-800";
  const vorschlaege = ["Südheide", "Celle", "Lüneburger Heide", "Harz", "Hannover"];

  return (
    <div className="mt-5 border-t border-stone-200 pt-4 dark:border-stone-700">
      <div className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-stone-800 dark:text-stone-100"><Search className="h-4 w-4 text-emerald-700 dark:text-emerald-300" /> Region durchsuchen</div>
      <p className="mt-1 text-xs text-stone-500 dark:text-stone-400">Findet die tatsächlich in OpenStreetMap eingetragenen {rad ? "Radrouten" : "Wanderrouten"} im Umkreis – auch lokale, die es nicht in die Favoriten oben schaffen (z. B. im Landkreis Celle).</p>

      <div className="mt-3 flex items-center gap-2">
        <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") run(); }}
          placeholder={rad ? "Region … z. B. Südheide, Celle" : "Region … z. B. Südheide, Hermannsburg"} className="w-full rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none dark:border-stone-700 dark:bg-stone-900" />
        <button onClick={() => run()} disabled={loading} className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-50">
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />} Suchen
        </button>
      </div>
      ...
      {err && <div className="mt-2 flex items-start gap-2 rounded-xl bg-rose-50 px-3 py-2 text-xs text-rose-700 dark:bg-rose-950 dark:text-rose-300"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /> {err}</div>}
      {loading && <div className="mt-3 flex items-center gap-2 text-sm text-stone-400 dark:text-stone-500"><Loader2 className="h-4 w-4 animate-spin" /> {rad ? "Radrouten" : "Wanderrouten"} werden gesucht …</div>}

      {data && (<>
        <div className="mt-3">
          <Wegkarte
            geojson={geom}
            spots={data.liste.slice(0, 60).map((r) => ({ id: r.id, name: r.name, lat: r.lat, lon: r.lon, kategorie: kat }))}
            selectedId={sel}
            onSelect={(id) => { const r = data.liste.find((x) => x.id === id); if (r) waehle(r); }}
            name={`${data.liste.length} ${rad ? "Radrouten" : "Wanderrouten"} um ${data.ort}`}
            hoehe="320px"
          />
          <div className="mt-1 text-center text-xs text-stone-400 dark:text-stone-500">Punkt = Mitte der Route (nicht der Start).</div>
        </div>

        <div className="mt-3 space-y-1.5">
          {data.liste.map((r) => {
            const aktiv = sel === r.id;
            return (
              <div key={r.id} id={zeilenId("rrs-", r.id)} className={"scroll-mt-24 rounded-lg px-2.5 py-2 text-sm transition " + (aktiv ? "bg-emerald-50 ring-1 ring-emerald-300 dark:bg-emerald-950 dark:ring-emerald-700" : "bg-stone-50 dark:bg-stone-800")}>
                <div className="flex items-center gap-2">
                  <button onClick={() => waehle(r)} className="min-w-0 flex-1 text-left">
                    <span className="flex flex-wrap items-center gap-1.5">
                      <span className="font-semibold text-stone-800 dark:text-stone-100">{r.name}</span>
                      {r.netz && <span className="rounded-full bg-stone-200 px-2 py-0.5 text-xs font-medium text-stone-600 dark:bg-stone-700 dark:text-stone-300">{r.netz}</span>}
                    </span>
                    <span className="mt-0.5 flex flex-wrap items-center gap-x-3 text-xs text-stone-500 dark:text-stone-400">
                      {r.km != null && <span className="inline-flex items-center gap-1"><Ruler className="h-3 w-3" /> {r.km} km</span>}
                      <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {r.weg} km ab {data.ort}</span>
                      {r.ref && <span>Nr. {r.ref}</span>}
                    </span>
                  </button>
                  {done[r.id]
                    ? <span className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"><Check className="h-3.5 w-3.5" /> {done[r.id]}</span>
                    : hatAktion && <button onClick={() => (nurIdeen ? (onAdd(sug(r)), merke(r, "in Ideen")) : setMenuFor(menuFor === r.id ? null : r.id))} className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-700 px-2.5 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-800">{menuFor === r.id ? <X className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />} {nurIdeen ? "Hinzufügen" : "Übernehmen"}</button>}
                </div>

                {aktiv && (
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-stone-500 dark:text-stone-400">
                    {geomInfo && geomInfo.id === r.id && geomInfo.status === "laedt" && <span className="inline-flex items-center gap-1"><Loader2 className="h-3 w-3 animate-spin" /> Verlauf wird geladen …</span>}
                    {geomInfo && geomInfo.id === r.id && geomInfo.status === "da" && <span className="inline-flex items-center gap-1 text-emerald-700 dark:text-emerald-300"><RouteIcon className="h-3 w-3" /> Verlauf gezeichnet.</span>}
                    {geomInfo && geomInfo.id === r.id && geomInfo.status === "leer" && <span>Verlauf nicht ladbar.</span>}
                    <a href={"https://www.openstreetmap.org/relation/" + r.id} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-stone-400 transition hover:text-emerald-700 dark:text-stone-500">auf OpenStreetMap ansehen <ExternalLink className="h-3 w-3" /></a>
                  </div>
                )}
                ...
                {menuFor === r.id && (
                  <div className="mt-2 space-y-1 rounded-lg border border-stone-200 bg-white p-2 dark:border-stone-700 dark:bg-stone-900">
                    {onAddToTrip && trips && trips.length > 0 && (<>
                      <div className="px-1 text-xs font-semibold uppercase tracking-wide text-stone-400 dark:text-stone-500">Zu einer Reise heften</div>
                      {trips.slice(0, 8).map((t) => (
                        <button key={t.id} onClick={() => { onAddToTrip(t.id, sug(r)); merke(r, "zu " + (t.name || "Reise")); }} className={zeile}>
                          <MapPin className="h-3.5 w-3.5 shrink-0 text-emerald-700 dark:text-emerald-300" /> <span className="min-w-0 truncate">{t.name || "Reise"}</span>
                        </button>
                      ))}
                    </>)}
                    {onCreateTrip && <button onClick={() => { onCreateTrip({ name: r.name, gebiet: (data && data.ort) || r.name, info: infoText(r), anreiseart: "auto", von: "Celle", nach: (data && data.ort) || "", items: [sug(r)] }); merke(r, "neue Reise"); }} className={zeile + " font-medium"}><CalendarPlus className="h-3.5 w-3.5 shrink-0 text-emerald-700 dark:text-emerald-300" /> Neue Reise daraus erstellen</button>}
                    {onAdd && <button onClick={() => { onAdd(sug(r)); merke(r, "in Ideen"); }} className={zeile}><Plus className="h-3.5 w-3.5 shrink-0 text-emerald-700 dark:text-emerald-300" /> In Ideen merken</button>}
                  </div>
                )}
              </div>
            );
          })}
          {data.liste.length === 0 && <div className="rounded-lg bg-stone-50 px-3 py-3 text-sm text-stone-500 dark:bg-stone-800 dark:text-stone-400">Keine benannte {rad ? "Radroute" : "Wanderroute"} in OpenStreetMap im Umkreis – größeren Umkreis wählen oder anderen Ort eingeben.</div>}
        </div>
        {data.voll && <div className="mt-2 text-xs text-stone-400 dark:text-stone-500">Liste bei 90 Treffern gekappt – kleineren Umkreis wählen für die nächstgelegenen.</div>}
      </>)}

      <div className="mt-3 flex items-start gap-2 text-xs text-stone-400 dark:text-stone-500"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>
        Live aus OpenStreetMap (ODbL) über Overpass, Verortung über Photon – frei und ohne Schlüssel. Gezeigt wird, was dort als benannte Route erfasst ist; Länge nur, wenn sie getaggt ist. Der Kartenpunkt ist die Mitte der Route. Ohne Gewähr.
      </span></div>
    </div>
  );
}
...
export default function Rundwege({ onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  async function waehle(w) {
    ...
    setSel(neu); setMenuFor(null);
    if (neu) scrollZuZeile(zeilenId("fuss-", w.key));
    if (!neu || lad[w.key] || busy) return;
    setBusy(w.key);
    let d = null, center = null;
    try {
      // 1) Region SOFORT verorten + Nadel setzen -> Karte springt gleich hin (vor dem Verlauf)
      try { center = await orten(w.r); if (center) setSpots((o) => ({ ...o, [w.key]: { lat: center.lat, lon: center.lon } })); } catch (e0) {}
      // 2) Verlauf laden (dauert länger) und zeichnen
      d = await ladeRoute(w, center);
      setLad((o) => ({ ...o, [w.key]: d }));
      setSpots((o) => ({ ...o, [w.key]: { lat: d.lat, lon: d.lon } }));  // exakter Startpunkt
      merkeWeg(w.key, d); setMerk(ladeMerk());
    } catch (e) { setLad((o) => ({ ...o, [w.key]: { fehler: e.message || String(e) } })); }
    finally { setBusy(null); }                      // Ladeanzeige weg, sobald die Linie da ist
    /* Höhenmeter GETRENNT nachladen – blockiert die Verlauf-Anzeige nicht mehr. */
    if (d) {
      setHmBusy(w.key);
      try {
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200">
      ...
      <CelleTouren art={art} onAdd={onAdd} onCreateTrip={onCreateTrip} trips={trips} onAddToTrip={onAddToTrip} />
      ...
      <div className="mt-3 space-y-1.5">
        {treffer.map((w) => {
          ...
          const h = hm[w.key];
          return (
            <div key={w.key} id={zeilenId("fuss-", w.key)} className={"scroll-mt-24 rounded-lg px-2.5 py-2 text-sm transition " + (aktiv ? "bg-emerald-50 ring-1 ring-emerald-300 dark:bg-emerald-950 dark:ring-emerald-700" : "bg-stone-50 dark:bg-stone-800")}>
              <div className="flex items-center gap-2">
      ...
      <RegionRoutenSuche art={art} onAdd={onAdd} onCreateTrip={onCreateTrip} trips={trips} onAddToTrip={onAddToTrip} />
