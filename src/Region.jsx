...
import { Compass, CloudSun, Search, MapPin } from "lucide-react";
...
export default function Region({ onAdd }) {
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200">
      ...
      <p className="mt-1 text-xs text-stone-500 dark:text-stone-400">Eine Region eingeben – Überblick (Infos & Sehenswürdigkeiten) und Vor Ort (Wetter, Luft, Fotos, Feiertage) in einem.</p>
      ...
      {q && (
        <div className="mt-3">
          ...
          {tab === "ueber"
            ? <Reiseberatung key={"r-" + q} embedded defaultQuery={q} onAdd={onAdd} />
            : <Bedingungen key={"b-" + q} embedded defaultQuery={q} />}
        </div>
      )}
    </section>
  );
