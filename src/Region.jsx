...
export default function Region({ onAdd, onCreateTrip, trips, onAddToTrip }) {
    ...
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200">
      ...
      {q && (
        <div className="mt-3">
          ...
          {tab === "wandern" && <Wandern key={"w-" + q} embedded defaultQuery={q} onAdd={onAdd} onCreateTrip={onCreateTrip} trips={trips} onAddToTrip={onAddToTrip} />}
        </div>
      )}
    </section>
  );
