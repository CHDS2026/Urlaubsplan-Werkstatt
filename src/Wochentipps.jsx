...
export default function Wochentipps() {
  ...
  useEffect(() => {
    const from = isoD(new Date()); const to = isoD(addDays(new Date(), 120));
  ...
  for (let i = 0; i < 12; i++) {
    const mon = addDays(mon0, i * 7), sun = addDays(mon, 6);
    ...
    weeks.push({ mon, sun, kw: isoWeek(mon), a, events, pubs, ferien, season: SEASON[mon.getMonth() + 1] });
  }
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200">
      ...
      <p className="mt-1 text-xs text-stone-500 dark:text-stone-400">Was lohnt sich wann ab Celle? Highlights, Ferien & Saison-Tipps für die nächsten Wochen. Anreise nach deinen Vorgaben (Auto ≤ 800 km, Direktflug nur EU ab HAJ, Direktzug nur ab Hannover).</p>
      ...
      <div className="mt-3 space-y-2">
        {weeks.map((w) => {
          const highlight = w.events.length > 0 || w.pubs.length > 0;
          return (
            <div key={w.a} className={"rounded-xl border p-3 " + (highlight ? "border-emerald-300 bg-emerald-50 dark:border-emerald-700 dark:bg-emerald-950" : "border-stone-200 dark:border-stone-700")}>
      ...
      <div className="mt-3 flex items-start gap-2 text-xs text-stone-400 dark:text-stone-500"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>MotoGP 2026 offiziell, <b>2027 vorläufig</b> (offizieller Kalender noch nicht veröffentlicht – bitte nach Bekanntgabe prüfen). Anreise: Auto nur bis ~800 km; Direktflüge nur innerhalb EU ab HAJ, Direktzüge nur ab Hannover – bei diesen Rennstrecken meist keine Direktverbindung, daher Auto oder „aufwändig". Ferien/Feiertage live (OpenHolidays). Ohne Gewähr.</span></div>
    </section>
  );
}
