...
import { Mountain, Search, Loader2, Info, ExternalLink, MapPin, Route as RouteIcon, Plus, Check, CalendarPlus, X, Clock, Repeat, Ruler } from "lucide-react";
...
const LAENGE = {
  ...
  kurz:   { label: "bis 10 km",   von: 0,  bis: 10 },
  mittel: { label: "10–25 km",    von: 10, bis: 25 },
  lang:   { label: "über 25 km",  von: 25, bis: 1e9 },
};
...
async function ladeWege(term, radiusKm, wunsch) {
  ...
  // Schritt 2: Länge für Kandidaten ohne "distance" aus der Geometrie berechnen.
  // Bewusst begrenzt: nur regionale/lokale Wege (Fernwege sprengen sonst die Abfrage), max. 14.
  const ohne = kand.filter((k) => k.km == null && k.rang <= 2).slice(0, 14);
  if (ohne.length) {
    try {
      const ids = ohne.map((k) => k.id).join(",");
      const o2 = await jpost("https://overpass-api.de/api/interpreter", `[out:json][timeout:60];relation(id:${ids});out geom;`, 45000);
      const map = {};
      for (const rel of ((o2 && o2.elements) || [])) { const L = laengeAusGeom(rel); if (L) map[rel.id] = L; }
      kand = kand.map((k) => (map[k.id] ? { ...k, km: Math.round(map[k.id] * 10) / 10, kmQuelle: "geom" } : k));
    } catch (e) { /* Länge bleibt unbekannt – ehrlicher als raten */ }
  }

  // Schritt 3: Passung bewerten
  const band = LAENGE[wunsch.laenge] || LAENGE.egal;
  const bewertet = kand.map((k) => {
    let p = 0;
    if (k.km != null) { p += (k.km >= band.von && k.km <= band.bis) ? 40 : -15; p += 8; }
    if (wunsch.rund && k.rund) p += 20;
    if (wunsch.rund && !k.rund) p -= 5;
    p += k.rang * 4;
    if (k.km != null && wunsch.laenge !== "egal") {
      const mitte = band.bis > 1e8 ? 35 : (band.von + band.bis) / 2;
      p -= Math.min(15, Math.abs(k.km - mitte) / 3);
    }
    return { ...k, punkte: p, zeit: gehzeit(k.km, k.hm) };
  });
  const gefiltert = bewertet.filter((k) => (!wunsch.nurWichtig || k.rang >= 2) && (wunsch.laenge === "egal" || k.km == null || (k.km >= band.von && k.km <= band.bis)));
  gefiltert.sort((a, b) => b.punkte - a.punkte || a.name.localeCompare(b.name, "de"));
  return { ort, lat, lon, wege: gefiltert, gesamt: kand.length };
}
...
export default function Wandern({ embedded = false, defaultQuery = "", onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  const [laenge, setLaenge] = useState("egal");
  const [rund, setRund] = useState(false);
  ...
  return (
    <section className={embedded ? "space-y-0 text-stone-800 dark:text-stone-200" : "rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200"}>
      ...
      <div className={(embedded ? "" : "mt-3 ") + "space-y-2 rounded-xl border border-stone-200 p-3 dark:border-stone-700"}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => { const v = !rund; setRund(v); neuLaden({ rund: v }); }} className={chip(rund)}><Repeat className="mr-1 inline h-3 w-3" />Rundweg bevorzugt</button>
      ...
      {data && (
        <div className="mt-3 rounded-xl border border-stone-200 p-3 dark:border-stone-700">
          <div className="mb-2 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700 dark:text-stone-200">
            <RouteIcon className="h-4 w-4 text-emerald-700 dark:text-emerald-300" /> Empfehlungen um {data.ort} · {data.wege.length} von {data.gesamt}
          </div>
      ...
      <div className="mt-3 flex items-start gap-2 text-xs text-stone-400 dark:text-stone-500"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Daten: OpenStreetMap/Overpass (ODbL), Suche via Photon – frei &amp; ohne Schlüssel. Länge aus dem OSM-Tag „distance“ oder aus der Geometrie berechnet (bei Fernwegen bleibt sie ggf. unbekannt). <b>Gehzeit ist eine Schätzung nach DIN 33466</b> (4 km/h, 300 Hm/h Aufstieg, ohne Pausen); Höhenmeter nur, wenn in OSM getaggt – sonst reine Flach-Schätzung. Es sind markierte OSM-Routen, keine fertigen Komoot-Touren. Ohne Gewähr.</span></div>
    </section>
  );
}
