...
const jget = (url, ms = 25000) => jgetBasis(url, ms); // längeres Zeitlimit für Overpass
...
function laengeAusGeom(rel) {
  ...
  for (const m of (rel.members || [])) {
    ...
    for (let i = 1; i < m.geometry.length; i++) sum += abstandKm(m.geometry[i - 1].lat, m.geometry[i - 1].lon, m.geometry[i].lat, m.geometry[i].lon);
  }
...
function rundAusGeom(rel) {
  ...
  return abstandKm(a.lat, a.lon, b.lat, b.lon) < 1;
}
