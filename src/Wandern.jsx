...
import { Mountain, Search, Loader2, Info, ExternalLink, MapPin, Route as RouteIcon, Plus, Check, CalendarPlus, X, Clock, Repeat, Ruler, Star, Map as MapIcon, Award, ChevronDown, ChevronUp } from "lucide-react";
import Wegkarte from "./Wegkarte.jsx";
import Rundwege from "./Rundwege.jsx";
...
export default function Wandern({ embedded = false, defaultQuery = "", onAdd, onCreateTrip, trips, onAddToTrip }) {
  ...
  const [rundAuf, setRundAuf] = useState(false);
  ...
  return (
    <section className={embedded ? "space-y-0 text-stone-800 dark:text-stone-200" : "rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900 text-stone-800 dark:text-stone-200"}>
      ...
      <div className={(embedded ? "" : "mt-3 ") + "space-y-2 rounded-xl border border-emerald-200 bg-emerald-50 p-3 dark:border-emerald-800 dark:bg-emerald-950"}>
        <button onClick={() => setRundAuf((v) => !v)} className="flex w-full items-center gap-2 rounded-lg bg-white px-3 py-2.5 text-left transition hover:bg-emerald-100 dark:bg-stone-900 dark:hover:bg-stone-800">
          <Award className="h-4 w-4 shrink-0 text-emerald-700 dark:text-emerald-300" />
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-bold text-stone-800 dark:text-stone-100">Schöne Rundwanderwege</span>
            <span className="block text-xs text-stone-500 dark:text-stone-400">Ausgezeichnete Tagestouren aus ganz Deutschland, auf der Karte</span>
          </span>
          {rundAuf ? <ChevronUp className="h-4 w-4 shrink-0 text-stone-400" /> : <ChevronDown className="h-4 w-4 shrink-0 text-stone-400" />}
        </button>
        {rundAuf && <div className="rounded-lg bg-white p-2 dark:bg-stone-900"><Rundwege onAdd={onAdd} onCreateTrip={onCreateTrip} trips={trips} onAddToTrip={onAddToTrip} /></div>}
