...
export default function Reisekarte({ trips, onOpenTrip, items = false, fit = false, spots, onSpotClick, selectedId, titel }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const t of trips || []) {
        ...
        if (coord && isFinite(coord.lat) && isFinite(coord.lon)) out.push({ id: t.id, name: t.name || t.region || "Reise", lat: coord.lat, lon: coord.lon, rolle: "reise" });
        if (items) {
          for (const it of (t.items || [])) {
            if (it.lat != null && it.lon != null && isFinite(Number(it.lat)) && isFinite(Number(it.lon))) out.push({ id: t.id, name: it.name || "", lat: Number(it.lat), lon: Number(it.lon), rolle: "aktivitaet", kategorie: it.kategorie });
          }
        }
      }
  ...
  useEffect(() => {
    ...
    pins.forEach((p) => {
      ...
      el.style.cssText = "line-height:0;cursor:" + (klickbar ? "pointer" : "default");
      if (istReise) {
        el.innerHTML = reiseMarker(gewaehlt);
        el.title = p.name || "Reise";
      } else {
        el.innerHTML = nadelSVG(p.kategorie, { aktiv: gewaehlt, size: 28 });
  ...
  return (
    <section className="mb-4 rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900">
      ...
      <div className="mt-2 text-xs text-stone-400 dark:text-stone-500">
        {fit ? "Reise = grauer Fähnchen-Punkt, Aktivitäten mit eigenem Symbol je Art (Berg, Rad, Wanderer, Kamera …). " : "Tippe eine Reise an, um sie zu öffnen. "}
