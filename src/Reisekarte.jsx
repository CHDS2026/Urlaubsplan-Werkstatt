...
import { MapPin, Loader2, Info, Map as MapIcon, Satellite } from "lucide-react";
...
export default function Reisekarte({ trips, onOpenTrip, items = false, fit = false }) {
  ...
  useEffect(() => {
    ...
    pins.forEach((p) => {
      ...
      if (p.main) {
        el.style.cssText = "width:22px;height:30px;cursor:" + (onOpenTrip ? "pointer" : "default");
      ...
      el.title = p.name || "";
      if (onOpenTrip) el.addEventListener("click", () => onOpenTrip(p.id));
  ...
  return (
    <section className="mb-4 rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900">
      ...
      <div className="mt-2 text-xs text-stone-400 dark:text-stone-500">
        ...
        Zoomen/Verschieben mit Fingern oder den +/−-Tasten.{modus === "sat" ? " Satellit: Sentinel-2 cloudless (© EOX, CC BY-NC-SA 4.0) – ca. 10 m Auflösung, gut für Landschaft, kein Straßenlevel." : ""}
      </div>
    </section>
  );
}
