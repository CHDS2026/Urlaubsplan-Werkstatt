...
const STYLE_KARTE = "https://tiles.openfreemap.org/styles/liberty";
const STYLE_SAT = {
  version: 8,
  sources: {
    sat: {
...
export default function Reisekarte({ trips, onOpenTrip, items = false, fit = false, spots, onSpotClick, selectedId, titel }) {
  ...
  useEffect(() => {
    if (spots) {
      setPins((spots || []).filter((p) => p.lat != null && p.lon != null && isFinite(Number(p.lat)) && isFinite(Number(p.lon))).map((p) => ({ id: p.id, name: p.name || "", lat: Number(p.lat), lon: Number(p.lon), main: true })));
      setLoading(false);
      return;
    }
    let alive = true;
      ...
      for (const t of trips || []) {
        ...
        if (items) {
          for (const it of (t.items || [])) {
            if (it.lat != null && it.lon != null && isFinite(Number(it.lat)) && isFinite(Number(it.lon))) out.push({ id: t.id, name: it.name || "", lat: Number(it.lat), lon: Number(it.lon), main: false });
          }
        }
      }
  ...
  useEffect(() => {
    ...
    pins.forEach((p) => {
      ...
      if (p.main) {
        el.style.cssText = "width:22px;height:30px;cursor:" + (onOpenTrip || onSpotClick ? "pointer" : "default");
        el.innerHTML = '<svg viewBox="0 0 18 26" width="22" height="30"><path d="M9 0C4 0 0 4 0 9c0 6.6 9 17 9 17s9-10.4 9-17c0-5-4-9-9-9z" fill="#047857" stroke="#fff" stroke-width="1.5"/><circle cx="9" cy="9" r="3.2" fill="#fff"/></svg>';
      } else {
        el.style.cssText = "width:12px;height:12px;border-radius:9999px;background:#0ea5e9;border:2px solid #fff;box-shadow:0 0 3px rgba(0,0,0,.4)";
      }
      el.title = p.name || "";
      if (p.main && selectedId && p.id === selectedId) el.style.filter = "drop-shadow(0 0 6px #047857)";
      if (onSpotClick) el.addEventListener("click", () => onSpotClick(p.id));
      else if (onOpenTrip) el.addEventListener("click", () => onOpenTrip(p.id));
      const mk = new maplibregl.Marker({ element: el, anchor: p.main ? "bottom" : "center" }).setLngLat([p.lon, p.lat]).addTo(map);
  ...
  return (
    <section className="mb-4 rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-800 dark:bg-stone-900">
      ...
      <div className="mt-2 text-xs text-stone-400 dark:text-stone-500">
        {fit ? "Grün = Ziel, blau = Programm-Punkte. " : "Tippe eine Nadel an, um die Reise zu öffnen. "}
        Zoomen/Verschieben mit Fingern oder den +/−-Tasten.{wege ? " „Wege“ zeigt markierte Wanderrouten (Waymarked Trails, ab ca. Zoom 8 sichtbar)." : ""}{modus === "sat" ? " Satellit: Sentinel-2 cloudless (© EOX, CC BY-NC-SA 4.0) – ca. 10 m Auflösung, gut für Landschaft, kein Straßenlevel." : ""}
      </div>
    </section>
  );
}
