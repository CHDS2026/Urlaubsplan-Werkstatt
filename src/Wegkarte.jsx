...
/* Nadel-Design je Kategorie — bewusst LOKAL definiert, damit Wegkarte unabhängig von
   der Reisekarte baut (ältere Reisekarte-Versionen exportieren nadelFuer nicht). Diese
   Tabelle muss mit dem NADEL in Reisekarte.jsx übereinstimmen, damit dieselbe Tour auf
   allen Karten gleich aussieht (Wandertour grün, Gipfel violett, Radtour blau). */
const NADEL = {
  wanderung:         { farbe: "#0f766e", pfad: "M9 4 L13 13 H5 Z", label: "Wandertour" },
  gipfel:            { farbe: "#7c3aed", pfad: "M9 3 L14 13 H4 Z", label: "Gipfeltour" },
  radtour:           { farbe: "#0284c7", pfad: "M5 11 h8 M9 11 V6", label: "Radtour" },
  fotospot:          { farbe: "#db2777", pfad: "M5 7 h8 v6 h-8 Z", label: "Fotospot" },
  aussicht:          { farbe: "#ca8a04", pfad: "M9 5 L13 12 H5 Z", label: "Aussicht" },
  restaurant:        { farbe: "#c2410c", pfad: "M7 5 v8 M11 5 v8", label: "Essen" },
  hotel:             { farbe: "#4f46e5", pfad: "M5 8 h8 v5 h-8 Z", label: "Unterkunft" },
  sehenswuerdigkeit: { farbe: "#047857", pfad: "", label: "Sehenswürdigkeit" },
};
...
export default function Wegkarte({ geojson, name, selectedId, onSelect, hoehe, spots }) {
  ...
  useEffect(() => {
    ...
    for (const p of spots || []) {
      ...
      const n = nadelFuer(p.kategorie);
      const innen = n.pfad
        ? '<path d="' + n.pfad + '" fill="none" stroke="#fff" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>'
        : '<circle cx="9" cy="9" r="3.2" fill="#fff"/>';
      /* Ausgewählt: gleiche Farbe (sonst ginge das Design verloren), dafür größer und mit
         rotem Schein – passend zur roten Linie des gewählten Verlaufs. */
      const bw = aktiv ? 24 : 20, bh = aktiv ? 32 : 27;
      const el = document.createElement("div");
      el.style.cssText = "width:" + bw + "px;height:" + bh + "px;cursor:pointer" + (aktiv ? ";filter:drop-shadow(0 0 5px #be123c)" : "");
      el.title = (p.name || "") + (n.label ? " · " + n.label : "");
      el.innerHTML = '<svg viewBox="0 0 18 26" width="' + bw + '" height="' + bh + '"><path d="M9 0C4 0 0 4 0 9c0 6.6 9 17 9 17s9-10.4 9-17c0-5-4-9-9-9z" fill="' + n.farbe + '" stroke="#fff" stroke-width="1.5"/>' + innen + '</svg>';
  ...
  /* Ohne Linie: auf die gewählte Nadel zoomen (bei Gipfeln gibt es keinen Verlauf). */
  useEffect(() => {
    const map = mapRef.current;
    if (!map || geojson || selectedId == null) return;
    const p = (spots || []).find((x) => x.id === selectedId);
    if (p && p.lat != null && p.lon != null) map.flyTo({ center: [p.lon, p.lat], zoom: Math.max(map.getZoom(), 12), duration: 700 });
