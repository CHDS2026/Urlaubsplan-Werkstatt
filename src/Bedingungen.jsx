...
export default function Bedingungen() {
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm">
      ...
      <p className="mt-1 text-xs text-stone-500">Wetter, Tageslicht, Luft & Pollen, Fotos, Feiertage und Touren für eine Region.</p>
      ...
      {place && (
        <div className="mt-3 space-y-3">
          <div className="text-xs text-stone-500">Für <b className="text-stone-800">{place.label}</b></div>

          {/* Wetter + Tageslicht */}
          {(wetter.loading || wetter.data) && (
            <div className="rounded-xl border border-stone-200 p-3">
              <div className="mb-2 flex items-center justify-between">
                <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700"><CloudSun className="h-4 w-4 text-amber-500" /> 14-Tage-Prognose</span>
                {wetter.data && (
              ...
              {wetter.loading && <div className="flex items-center gap-2 text-sm text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> lädt …</div>}
              {wetter.data && (
                <div className="flex gap-1.5 overflow-x-auto pb-1">
                  {wetter.data.days.map((d, i) => {
                    const [emo] = wmo(d.code); const wd = WT[new Date(d.date + "T00:00:00").getDay()];
                    return (
                      <div key={i} className="shrink-0 rounded-lg bg-stone-50 px-2 py-1.5 text-center" style={{ minWidth: "44px" }}>
                        <div className="text-xs text-stone-400">{wd}</div>
                        <div className="text-base leading-tight">{emo}</div>
                        <div className="text-xs font-semibold text-stone-800">{Math.round(d.tmax)}°</div>
                        <div className="text-xs text-stone-400">{Math.round(d.tmin)}°</div>
          ...
          {(luft.loading || luft.data) && (
            <div className="rounded-xl border border-stone-200 p-3">
              <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700"><Wind className="h-4 w-4 text-emerald-700" /> Luft & Pollen</span>
              {luft.loading && <div className="mt-1 flex items-center gap-2 text-sm text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> lädt …</div>}
              {luft.data && (
                <div className="mt-1.5 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {luft.data.pm25 != null && <span className="rounded-lg bg-stone-100 px-2 py-1 text-stone-600">PM2,5 {Math.round(luft.data.pm25)}</span>}
                  {luft.data.pollen.length === 0 && <span className="text-stone-400">keine relevanten Pollen</span>}
          ...
          {(fotos.loading || (fotos.data && fotos.data.length > 0)) && (
            <div className="rounded-xl border border-stone-200 p-3">
              <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700"><Camera className="h-4 w-4 text-emerald-700" /> Fotos aus der Gegend</span>
              {fotos.loading && <div className="mt-1 flex items-center gap-2 text-sm text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> lädt …</div>}
              {fotos.data && fotos.data.length > 0 && (
                <>
                  ...
                  <div className="mt-1 text-xs text-stone-400">Wikimedia Commons · Lizenzen je Bild (aufs Bild tippen).</div>
                </>
              )}
            </div>
          )}
          ...
          {(feier.loading || (feier.data && feier.data.length > 0)) && (
            <div className="rounded-xl border border-stone-200 p-3">
              <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700"><CalendarDays className="h-4 w-4 text-emerald-700" /> Feiertage am Ziel {place.cc ? "(" + place.cc + ")" : ""}</span>
              <div className="mt-0.5 text-xs text-stone-400">Nächste ~90 Tage – für Öffnungszeiten & Andrang.</div>
              {feier.loading && <div className="mt-1 flex items-center gap-2 text-sm text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> lädt …</div>}
              {feier.data && feier.data.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {feier.data.map((f, i) => (
                    <span key={i} className="inline-flex items-center gap-1 rounded-lg bg-stone-50 border border-stone-200 px-2 py-1 text-xs text-stone-600"><b className="text-stone-800">{dmy(f.date)}</b> {f.name}</span>
                  ))}
                </div>
              )}
            </div>
          ...
          {(routen.loading || (routen.data && routen.data.length > 0)) && (
            <div className="rounded-xl border border-stone-200 p-3">
              <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-stone-700"><Route className="h-4 w-4 text-emerald-700" /> Wander- & Radrouten</span>
              {routen.loading && <div className="mt-1 flex items-center gap-2 text-sm text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> lädt …</div>}
              {routen.data && routen.data.length > 0 && (
                <ul className="mt-2 space-y-1.5">
                  {routen.data.map((r, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm">
                      <span className="rounded bg-stone-100 px-1.5 py-0.5 text-xs font-medium text-stone-500">{r.typ}</span>
          ...
          <div className="flex items-start gap-2 text-xs text-stone-400"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Wetter/Luft/Pollen: Open-Meteo · Fotos: Wikimedia Commons · Feiertage: OpenHolidays · Routen: OSM/Overpass. Prognose ist Vorhersage, keine Garantie.</span></div>
        </div>
      )}
    </section>
  );
