...
import { geocode } from "./lib/orte.js";
import { abstandKmGerundet } from "./lib/abstand.js";
...
async function ladeThema(theme, center, radiusKm) {
  if (theme.curated) {
    return MOTOGP.map((c) => ({ ...c, distCenter: abstandKmGerundet(center.lat, center.lon, c.lat, c.long) }))
  ...
  for (const el of (j.elements || [])) {
    ...
    out.push({ name, lat, long: lon, distCenter: abstandKmGerundet(center.lat, center.lon, lat, lon) });
  }
...
export default function Entdecken({ onAdd }) {
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-white dark:bg-stone-900 p-4 shadow-sm">
      ...
      <p className="mt-1 text-xs text-stone-500 dark:text-stone-400">Region eingeben, Thema wählen – passende Orte im Umkreis, mit Distanz und Fahrzeit ab Celle. Ein Klick übernimmt sie in die Ideen.</p>
      ...
      {center && (
        <div className="mt-3">
          <div className="flex items-center justify-between gap-3">
            <span className="text-sm text-stone-600 dark:text-stone-300">Umkreis um <b className="text-stone-800 dark:text-stone-100">{center.label}</b></span>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {THEMEN.map((t) => (
              <button key={t.key} onClick={() => waehle(t)}
                className={"rounded-full px-3 py-1.5 text-xs font-medium transition " + (theme === t.key ? "bg-emerald-700 text-white" : "bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 hover:bg-stone-200")}>
                {t.label}
              </button>
            ))}
          </div>

          {res.loading && <div className="mt-3 flex items-center gap-2 rounded-xl bg-stone-50 dark:bg-stone-800 px-3 py-3 text-sm text-stone-500 dark:text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> suche „{activeTheme ? activeTheme.label : ""}“ …</div>}
          ...
          {!res.loading && theme && (
            <div className="mt-3">
              {res.list.length === 0 && !res.error && <p className="text-sm text-stone-400 dark:text-stone-500">Nichts im Umkreis gefunden – Radius erhöhen oder anderes Thema.</p>}
                  ...
                  return (
                    <li key={i} className="flex items-start gap-3 rounded-xl border border-stone-100 dark:border-stone-800 p-3">
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-semibold text-stone-800 dark:text-stone-100">{it.name}</div>
