...
const BEREICHE = [
  { k: "reisen", label: "Reisen", Icon: Calendar, txt: "Tage & Programm, Karte, Budget, Packliste, Dokumente, PDF." },
  { k: "ziele", label: "Ziele & Ideen", Icon: Globe2, txt: "Gemerkte Ausflugsziele und Touren auf der Ideen-Karte." },
  { k: "bestpreis", label: "Bestpreis", Icon: Wallet, txt: "Preisübersicht zu deinen Reisen." },
  { k: "tools", label: "Tools", Icon: Compass, txt: "Ideen finden · Ziel prüfen · Anreise & Preise." },
];
...
export default function Startseite({ onGo, trips, ideen, onOpenTrip, onCreate }) {
  ...
  const kommend = (trips || []).filter((t) => t.start && t.end && t.end >= heute && !t.erledigt).sort((a, b) => (a.start < b.start ? -1 : 1));
  ...
  const kachel = "flex w-full items-start gap-3 rounded-2xl border border-stone-200 bg-white p-3 text-left shadow-sm transition hover:border-emerald-300 hover:shadow-md dark:border-stone-700 dark:bg-stone-900 dark:hover:border-emerald-700";
    ...
    <div className="space-y-4">
      ...
      <section>
        ...
        <div className="grid gap-2">
          {BEREICHE.map((b) => (
            <button key={b.k} onClick={() => geh(b.k)} className={kachel}>
