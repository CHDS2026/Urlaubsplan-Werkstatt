...
import { Plane, Train, Search, ExternalLink, Globe2, Sparkles, Info, CalendarDays, Sun, Tag, Check, X, Luggage, ChevronRight } from "lucide-react";
import { db } from "./db.js";
import { ladeSchulferien, ferienAnzahl, reisezeitStufe } from "./ferien.js";
...
function KalenderView({ mode, ziel, preise, setzePreis, trips, onOpenTrip }) {
  ...
  useEffect(() => {
    let aktiv = true;
    ladeSchulferien(start, addDays(start, 27)).then((m) => {
      if (!aktiv) return;
  ...
  // Eigene Preise haben Vorrang; sonst Einfärbung nach Ferienlage
  const farbe = (p, tag) => {
    if (typeof p === "number") {
    ...
    if (stufe === "mittel") return "bg-white text-stone-600";
  ...
  const guenstigeTage = ferien ? tage.filter((t) => reisezeitStufe(ferien, t) === "guenstig").length : 0;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
        ...
        <span>Grün = <strong>kein Bundesland hat Schulferien</strong> (erfahrungsgemäß günstiger), rot = viele Bundesländer in Ferien. Tag antippen öffnet die Suche. Eigene Preise kannst du notieren – sie überschreiben die Farbe.</span>
      </div>
      ...
      <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className="text-xs text-stone-500">{min != null ? `ab ${min} €` : ferienStatus === "ok" ? `${guenstigeTage} ferienfreie Tage` : ""}</span>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {tage.map((t) => {
            ...
            const titel = reise ? `Reise: ${reise.name || "Ohne Titel"}` : ft ? ft : n ? `${n} Bundesländer in Ferien` : "keine Schulferien";
            return (
              <button key={t} onClick={() => oeffne(t)} onContextMenu={(e) => { e.preventDefault(); setNotiz(t); setWert(p != null ? String(p) : ""); }}
                title={titel}
                className={`relative flex h-14 flex-col items-center justify-center rounded-lg border text-xs transition ${farbe(p, t)} ${reise ? "border-sky-400" : we ? "border-emerald-300" : "border-stone-200"} hover:border-emerald-400`}>
                {reise && <span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-sky-500" />}
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-stone-500">
          <span className="inline-flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm bg-emerald-50 border border-emerald-300" /> ferienfrei</span>
          <span className="inline-flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm bg-rose-50 border border-rose-300" /> Hauptreisezeit</span>
        ...
        <p className="mt-2 text-xs text-stone-400">Zahl = Bundesländer mit Ferien · FT = Feiertag. Preis notieren: Tag lange drücken oder unten wählen.</p>
      ...
      {reisenImZeitraum.length > 0 && (
        <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
          <h3 className="mb-2 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-stone-800"><Luggage className="h-4 w-4 text-sky-600" /> Deine Reisen in diesem Zeitraum</h3>
...
function WochenendeView({ mode, ziel, preise, trips, onOpenTrip }) {
  ...
  const vorschlaege = freitage.map((fr) => {
    ...
    return { beginn, ende, naechte, anlass, lang: Boolean(moFeiertag || frFeiertag), ferienfrei, ferienLaender, belegt };
  });
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {vorschlaege.map((t) => {
          ...
          return (
            <div key={t.beginn} className={`rounded-2xl border bg-white p-4 shadow-sm ${t.belegt ? "border-sky-300" : t.lang ? "border-emerald-300" : "border-stone-200"}`}>
              <button onClick={() => oeffnen(t)} disabled={!ziel.trim()} className="w-full text-left disabled:opacity-50">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    {t.ferienfrei && <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700">ferienfrei</span>}
