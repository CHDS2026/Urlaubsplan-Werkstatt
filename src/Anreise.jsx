...
import { geocode as geocodeBasis } from "./lib/orte.js";
import { abstandKmGerundet } from "./lib/abstand.js";
const geocode = (q) => geocodeBasis(q, "Ziel"); // Wortlaut der Fehlermeldung wie bisher
...
function flugCheck(to) {
  let best = null;
  for (const a of HAJ) { const d = abstandKmGerundet(to.lat, to.lon, a.lat, a.lon); if (!best || d < best.d) best = { ...a, d }; }
  if (best && best.d <= 150) return { ...best, direkt: true, flugKm: abstandKmGerundet(HAJ_APT.lat, HAJ_APT.lon, best.lat, best.lon) };
  return { direkt: false };
}
...
export default function Anreise() {
  ...
  const airKm = dest ? abstandKmGerundet(CELLE.lat, CELLE.lon, dest.lat, dest.lon) : 0;
