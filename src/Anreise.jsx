...
export default function Anreise() {
  ...
  const num = "w-16 rounded-lg border border-stone-200 bg-white px-2 py-1 text-sm focus:border-emerald-400 focus:outline-none";
  ...
  return (
    <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm">
      ...
      <p className="mt-1 text-xs text-stone-500">Auto, Bahn und Direktflug ab HAJ im Vergleich – mit transparenter Kosten-/CO₂-Schätzung nach deinen Annahmen.</p>

      <div className="mt-3 flex items-center gap-2">
        <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") run(); }}
          placeholder="Ziel (Region oder Stadt) …" className="w-full rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none" />
      ...
      {/* Annahmen */}
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-xl bg-stone-50 px-3 py-2 text-xs text-stone-600">
        <span className="inline-flex items-center gap-1"><Fuel className="h-3.5 w-3.5 text-stone-400" /> Verbrauch <input type="number" min={1} step={0.5} value={verbrauch} onChange={(e) => setVerbrauch(Number(e.target.value) || 0)} className={num} /> L/100</span>
        ...
        <span className="inline-flex items-center gap-1"><Users className="h-3.5 w-3.5 text-stone-400" /> Personen <input type="number" min={1} step={1} value={personen} onChange={(e) => setPersonen(Math.max(1, Number(e.target.value) || 1))} className={num} /></span>
      </div>

      {dest && (
        <div className="mt-3 space-y-3">
          <div className="text-xs text-stone-500">Ziel: <b className="text-stone-800">{dest.label}</b> · Angaben je <b>einfache Strecke</b>.</div>

          {/* AUTO */}
          <div className="rounded-xl border border-stone-200 p-3">
            <div className="flex items-center justify-between">
              <span className="inline-flex items-center gap-2 font-semibold text-stone-800"><Car className="h-4 w-4 text-emerald-700" /> Auto<Badge show={fastest === "auto"} kind="fast" /><Badge show={greenest === "auto"} kind="co2" /></span>
            ...
            {auto.loading && <div className="mt-1 flex items-center gap-2 text-sm text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> Route …</div>}
            {auto.err && <div className="mt-1 text-sm text-stone-400">Route nicht ermittelbar ({auto.err}).</div>}
            {auto.data && (
              <div className="mt-1 grid grid-cols-3 gap-2 text-sm">
                <div><div className="text-xs text-stone-400">Dauer</div><div className="font-semibold text-stone-800">{hm(auto.data.min)}</div><div className="text-xs text-stone-400">{auto.data.km} km</div></div>
                <div><div className="text-xs text-stone-400">Sprit (gesamt)</div><div className="font-semibold text-stone-800">{eur(autoKostenGes)}</div><div className="text-xs text-stone-400">{eur(autoKostenGes / personen)}/Person</div></div>
                <div><div className="text-xs text-stone-400 inline-flex items-center gap-1"><Leaf className="h-3 w-3" />CO₂</div><div className="font-semibold text-stone-800">{kg(autoCO2)}</div><div className="text-xs text-stone-400">{kg(autoCO2 / personen)}/Person</div></div>
              </div>
            )}
          </div>
          ...
          {/* BAHN */}
          <div className="rounded-xl border border-stone-200 p-3">
            <span className="inline-flex items-center gap-2 font-semibold text-stone-800"><Train className="h-4 w-4 text-emerald-700" /> Bahn<Badge show={fastest === "bahn"} kind="fast" /><Badge show={greenest === "bahn"} kind="co2" /></span>
            {bahn.loading && <div className="mt-1 flex items-center gap-2 text-sm text-stone-400"><Loader2 className="h-4 w-4 animate-spin" /> Verbindung …</div>}
            {bahn.err && <div className="mt-1 text-sm text-stone-400">Keine Verbindung ermittelbar ({bahn.err}).</div>}
            {bahn.data && (
              <div className="mt-1 grid grid-cols-3 gap-2 text-sm">
                <div><div className="text-xs text-stone-400">Schnellste</div><div className="font-semibold text-stone-800">{hm(bahn.data.min)}</div><div className="text-xs text-stone-400 truncate">bis {bahn.data.station}</div></div>
                <div><div className="text-xs text-stone-400">Umstiege</div><div className="font-semibold text-stone-800">{bahn.data.umst === 0 ? "direkt" : bahn.data.umst}</div></div>
                <div><div className="text-xs text-stone-400 inline-flex items-center gap-1"><Leaf className="h-3 w-3" />CO₂</div><div className="font-semibold text-stone-800">{kg(bahnCO2)}</div><div className="text-xs text-stone-400">/Person</div></div>
              </div>
            )}
            <div className="mt-1 text-xs text-stone-400">Preis stark variabel – Sparpreis/Deutschland-Ticket auf <a href="https://www.bahn.de" target="_blank" rel="noreferrer" className="text-emerald-700 hover:text-emerald-800">bahn.de</a> prüfen.</div>
          </div>

          {/* FLUG */}
          <div className="rounded-xl border border-stone-200 p-3">
            <span className="inline-flex items-center gap-2 font-semibold text-stone-800"><Plane className="h-4 w-4 text-emerald-700" /> Flug ab HAJ<Badge show={greenest === "flug"} kind="co2" /></span>
            {flug && flug.direkt ? (
              <div className="mt-1 text-sm">
                <div className="text-stone-800"><b>Direktflug</b> nach {flug.name} <span className="font-mono text-xs text-stone-500">{flug.iata}</span> · {flug.airline} <span className="text-stone-400">({flug.saison})</span></div>
                <div className="mt-1 grid grid-cols-3 gap-2">
                  <div><div className="text-xs text-stone-400">Flughafen</div><div className="text-sm font-semibold text-stone-800">{flug.iata}</div><div className="text-xs text-stone-400">~{flug.d} km vom Ziel</div></div>
                  <div><div className="text-xs text-stone-400 inline-flex items-center gap-1"><Leaf className="h-3 w-3" />CO₂</div><div className="text-sm font-semibold text-stone-800">{kg(flugCO2)}</div><div className="text-xs text-stone-400">/Person</div></div>
              ...
              <div className="mt-1 text-sm text-stone-500">Kein Direktflug ab HAJ in Zielnähe (in der kuratierten Auswahl). Volle Karte: <a href="https://www.hannover-airport.de/rund-ums-fliegen/direktziele/" target="_blank" rel="noreferrer" className="text-emerald-700 hover:text-emerald-800">hannover-airport.de</a>.</div>
            )}
          </div>

          <div className="flex items-start gap-2 text-xs text-stone-400"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Kosten/CO₂ sind Schätzungen (Auto {"~"}150 g/km, Bahn {"~"}32 g/Pkm, Flug {"~"}180 g/Pkm; Auto-Kosten nach deinen Werten). Bahn-/Flugpreise variabel – bewusst nicht geschätzt. Quellen: OSRM, DB (v6.db.transport.rest), Photon/OSM.</span></div>
        </div>
      )}
    </section>
  );
