...
export default function TripMap({ punkte, mitLinie = true, klickModus = false, onKarteKlick, zentrum }) {
  ...
  useEffect(() => {
    ...
    // Ohne eigene Punkte: auf die Reise-Region zentrieren (zentrum = [lon, lat]), sonst Deutschland.
    const center = mitKoord.length ? [mitKoord[0].lon, mitKoord[0].lat] : (zentrum && zentrum.length === 2 ? zentrum : [10.45, 51.16]);

    karte.current = new maplibregl.Map({
      container: box.current,
      style: STYLE,
      center,
      zoom: mitKoord.length ? 8 : (zentrum && zentrum.length === 2 ? 10 : 4),
