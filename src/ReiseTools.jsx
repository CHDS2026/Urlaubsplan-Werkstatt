...
import { Lightbulb, Sparkles, CalendarDays, CalendarRange, Compass, ArrowLeftRight, Plane, Thermometer, Award, ChevronLeft, ChevronRight, Tag, Car, WifiOff } from "lucide-react";
...
import Offlinekarte from "./Offlinekarte.jsx";
...
export const GROUPS = [
  {
    ...
    items: [
      ...
      { k: "touren", label: "Schöne Touren", desc: "Favoriten nach Bundesland – Wandern & Rad, auf der Karte", Icon: Award, C: Rundwege, add: true, trip: true, attach: true },
  ...
  },
  {
    ...
    items: [
      ...
      { k: "offline", label: "Offline-Karten", desc: "Region vorladen für unterwegs", Icon: WifiOff, C: Offlinekarte, add: false },
    ],
  },
  {
...
export default function ReiseTools({ onAdd, active, onOpen, onCreateTrip, trips, onAddToTrip }) {
  ...
  return (
    <div className="w-full space-y-5 text-stone-800 dark:text-stone-200">
                <div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-4 shadow-sm dark:border-emerald-800 dark:from-emerald-950 dark:to-stone-900">
            ...
            {grp.items.map((it) => {
              ...
              return (
                <button key={it.k} onClick={() => setKey(it.k)}
                  className="flex w-full items-center gap-3 rounded-2xl border border-stone-200 bg-white p-3 text-left shadow-sm transition hover:-translate-y-px hover:border-emerald-300 hover:bg-emerald-50 hover:shadow-md dark:border-stone-700 dark:bg-stone-900 dark:hover:border-emerald-700 dark:hover:bg-stone-800">
